...
{T_B
	...
	CCharacter* pCCha = g_pScriptMap->ChaSpawn(nScriptID, enumCHACTRL_NONE, sAngle, &Pos, false, nullptr, 12000, lReliveTime);
	if (pCCha)
	{
...
{T_B
	...
	if (CCharacter* pCCha = pMainCha->m_submap->ChaSpawn(nScriptID, enumCHACTRL_NONE, sAngle, &Pos, false, nullptr, 12000, lReliveTime); pCCha)
	{
		pCCha->SetResumeTime(lReliveTime * 1000);
		lua_pushlightuserdata(L, pCCha);
		return 1;
...
{T_B
	...
	CCharacter* pCCha = pMap->ChaSpawn(nScriptID, enumCHACTRL_NONE, sAngle, &Pos, false, nullptr, 12000, lReliveTime);
	if (pCCha)
	{
		pCCha->SetResumeTime(lReliveTime * 1000);
		lua_pushlightuserdata(L, pCCha);
