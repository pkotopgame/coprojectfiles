...
public:
	...
	// bosses respawn controller @mothannakh
	struct BossesInfo {
		unsigned int characterId{ 0 };  // monster id
		unsigned int respawntime{ 0 };  // monster default respawn in seconds
		unsigned int respawnTicks{ 0 }; // next expected respawn
		unsigned int PoseX{ 0 };		  // pose cord x
		unsigned int PoseY{ 0 };		  // pose cord y
		unsigned int Angle{ 0 };		  // look direction
		std::string MapName;

		BossesInfo() = default;
	};
	BOOL IsBossTrackerActive{ FALSE };
	std::unordered_set<unsigned int> BossesRespawnIDS{}; //bosses we should track them on die actions 
	std::unordered_set<unsigned int> BossesShouldRespawn{}; //bosses should respawn later 
	std::map<unsigned int, BossesInfo>BossRespawnInfo{}; //containt monster id as key, and monster struct info
	bool GameServerFirstRun{ true }; // to run chaspawn checks only for first startup of gameserver 
	//bosses respawn end
};
