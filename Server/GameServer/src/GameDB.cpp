...
cChar* CTableCha::GetPlayerNameByRoleID(const uLong Cha_id) {
	string buf[2];
	char filter[80];
...
std::string CTableCha::GetName(const int cha_id)
{
	string buf[1];
...
bool CTableMaster::IsMasterRelation(const int masterID, const int prenticeID)
{
	char param[] = "count(*)";
	char filter[80]; 
...
bool CTableCha::ShowExpRank(CCharacter* pCha, const int count)
{
	bool ret = false;
...
// ��ɫ������Ϸʱ��ȡ���� 
//-----------------------
bool CTableCha::ReadAllData(CPlayer *pPlayer, const DWORD atorID)
{T_B
	if(!pPlayer)
	{
...
// ��ʱ��ɫ��Ϣ����
//-----------------
bool CTableCha::SaveAllData(CPlayer *pPlayer, const char chSaveType)
{T_B
...
BOOL CTableCha::SaveStoreItemID(const DWORD atorID, const long lStoreItemID)
{
	if(atorID == 0)
	{
		return false;
...
BOOL CTableCha::AddMoney(const DWORD atorID, const long money)
{
	if(atorID == 0)
	{
		return false;
...
BOOL CTableCha::AddCreditByDBID(const DWORD atorID, const long lCredit)
{
	if(atorID == 0)
	{
		return false;
...
BOOL CTableCha::IsChaOnline(const DWORD atorID, BOOL &bOnline)
{
	if(atorID == 0)
	{
		return false;
...
Long CTableCha::GetChaAddr(const DWORD atorID)
{
	if(atorID == 0)
		return false;
...
bool CTableCha::SaveTableVer(const DWORD atorID)
{
	sprintf(g_sql, "update %s set \
				version=%d \
				where atorID=%d", \
...
BOOL CTableCha::SaveMissionData(CPlayer *pPlayer, const DWORD atorID)
{T_B
...
bool CTableLotterySetting::AddIssue(const int issue)
{
...
bool CTableLotterySetting::DisuseIssue(const int issue, const int state)
{
...
bool CTableLotterySetting::SetWinItemNo(const int issue, const char* itemno)
{
...
bool CTableLotterySetting::GetWinItemNo(const int issue, string& itemno)
{
	string buf[1]; 
	char param[] = "itemno";
	char filter[80]; 
...
bool CTableTicket::AddTicket(const int atorID, const int issue, char itemno[6][2])
{
	int index = -1;
	char no[10][6];
...
bool CTableTicket::AddTicket(const int atorID, const int issue, const char itemno1, const char itemno2, const char itemno3, const char itemno4, const char itemno5, const char itemno6, const int real)
{
...
bool CTableTicket::IsExist(const int issue, char* itemno)
{
	string buf[1]; 
	char param[] = "count(*)";
	char filter[50]; 
...
bool CTableTicket::CalWinTicket(const int issue, const int max, string& itemno)
{
	int RANGE_MIN = 1;
    int RANGE_MAX = 2;
...
bool CTableWinTicket::AddTicket(const int issue, char* itemno, const int grade)
{
...
bool CTableWinTicket::Exchange(const int issue, char* itemno)
{
...
// ׷��һ������
bool CTableAmphitheaterSetting::AddSeason(const int season)
{
...
// ��������״̬
bool CTableAmphitheaterSetting::DisuseSeason(const int season, const int state,const char* winner)
{
...
// �����ִ�
bool CTableAmphitheaterSetting::UpdateRound(const int season, const int round)
{
...
// ����ע�� ����ע����teamID
bool CTableAmphitheaterTeam::TeamSignUP(int &teamID, const int captain, const int member1, const int member2)
{
	// ȡ�ÿհ׶���ID
	if(teamID < 0)
	{
...
// ����ȡ��
bool CTableAmphitheaterTeam::TeamCancel(const int teamID)
{
...
// �ж���Ч�Ķ���
bool CTableAmphitheaterTeam::IsValidAmphitheaterTeam(const int teamID, const int captainID, const int member1, const int member2)
{
	string buf[1]; 
	char param[] = "count(*)";
	char filter[80]; 
...
//�ж��Ƿ��Ѿ�ע��
bool CTableAmphitheaterTeam::IsLogin(const int pActorID)
{
	string buf[1];
	char param[] = "count(*)";
	char filter[80];
...
//�ж��Ƿ�õ�ͼ�������
bool CTableAmphitheaterTeam::IsMapFull(const int MapID,int & PActorIDNum)
{
	string buf[1];
	char param[] = "count(*)";
	char filter[80];
...
//���µ�ͼmapflag
bool CTableAmphitheaterTeam::UpdateMapNum(const int Teamid, const int Mapid, const int MapFlag)
{
...
//���mapflagֵ
bool CTableAmphitheaterTeam::GetMapFlag(const int Teamid,int & Mapflag)
{
	string buf[1];
	char param[] = "mapflag";
	char filter[80];
...
//���ñ��������״̬
bool CTableAmphitheaterTeam::SetMatchResult(const int Teamid1, const int Teamid2, const int Id1state, const int Id2state)
{
...
//����mapid ȡ�õ�ͼ�������ӳ���id
bool CTableAmphitheaterTeam::GetCaptainByMapId(const int Mapid,string &Captainid1,string &Captainid2)
{
	string NoCaptain = "";
...
//����map�ֶ�
bool CTableAmphitheaterTeam::UpdateMap(const int Mapid)
{
...
//����Ʊ��
bool CTableAmphitheaterTeam::UpdatReliveNum(const int ReID )
{
	string buf[1];
...
//���¶�������ͼ���map�ֶ�
bool CTableAmphitheaterTeam::UpdateMapAfterEnter(const int CaptainID, const int MapID)
{
...
//����winnum��ֵ��ʤ����1
bool CTableAmphitheaterTeam::UpdateWinnum(const int teamid )
{
...
bool CTableAmphitheaterTeam::SetMatchnoState(const int teamid )
{
...
bool CTableAmphitheaterTeam::CleanMapFlag(const int teamid1, const int teamid2)
{
...
bool CTableAmphitheaterTeam::GetStateByTeamid(const int teamid, int &state )
{
	string buf[1];
	char param[] = "state";
	char filter[80];
...
//��ȡbirthday
bool CTablePersoninfo::GetPersonBirthday(const int chaid, int &birthday )
{
	string buf[1];
	char param[] = "birthday";
	char filter[80];
...
bool CTableResource::Create(long &lDBID, const long lChaId, const long lTypeId)
{
...
bool CTableResource::ReadKitbagTmpData(const long lRecDBID, string& strData)
{
	if(lRecDBID == 0)
	{
		return false;
...
bool CTableResource::SaveKitbagTmpData(const long lRecDBID, const string& strData)
{
	if(lRecDBID == 0)
	{
		return false;
...
bool CTableResource::ReadBankData(CPlayer *pCPly, const char chBankNO)
{
	if (!pCPly)
	{
		//LG("enter_map", "����Դ���ݿ���󣬽�ɫ������.\n");
...
bool CTableResource::SaveBankData(CPlayer *pCPly, const char chBankNO)
{
...
bool CTableMapMask::GetColNameByMapName(const char *szMapName, char *szColName, const long lColNameLen)
{
...
bool CTableMapMask::Create(long &lDBID, const long lChaId)
{
...
bool CTableMapMask::SaveData(CPlayer *pCPly, const BOOL bDirect)
{
	if(!pCPly || !pCPly->IsValid()) return false;
...
bool CTableAct::ReadAllData(CPlayer *pPlayer, const DWORD ato_id)
{T_B
	if(!pPlayer) return false;
...
BOOL CTableBoat::SaveBoatTempData(const DWORD dwBoatID, const DWORD dwOwnerID, const BYTE byIsDeleted )
{
...
BOOL CTableBoat::SaveBoatDelTag(const DWORD dwBoatID, const BYTE byIsDeleted )
{
...
BOOL CTableBoat::SaveBoatTempData( CCharacter& Boat, const BYTE byIsDeleted )
{
	DWORD dwBoatID = (DWORD)Boat.getAttr( ATTR_BOAT_DBID );
...
BOOL CTableBoat::SaveBoat( CCharacter& Boat, const char chSaveType )
{T_B
...
bool CTableBoat::SaveAllData(CPlayer *pPlayer, const char chSaveType)
{
	if (!pPlayer)
		return false;
	CCharacter	*pCBoat;
...
bool CTableBoat::SaveAllCabin(CPlayer *pPlayer, const char chSaveType)
{
	CCharacter	*pCBoat;
	for (BYTE i = 0; i < pPlayer->GetNumBoat(); i++)
	{
...
{T_B
	...
	printf_s("Connecting database [%s : %s]... ", g_Config.m_szDBIP, g_Config.m_szDBName);
	...
	//boses respawn tracker 
	_tab_BossesRespawn = new CTableBossesRespawn(&_connect);
	// Modify by lark.li 20080521
	//if (!_tab_cha || !_tab_act || !_tab_gld || !_tab_boat || !_tab_master)
	if (!_tab_cha || !_tab_act || !_tab_gld || !_tab_boat || !_tab_master 
			|| !_tab_setting || !_tab_ticket || !_tab_winticket
			|| !_tab_amp_setting || !_tab_amp_team || !_tab_log || !_tab_item ||!_tab_BossesRespawn)
		return FALSE;
	...
	if (!_tab_cha->Init() || !_tab_act->Init() || !_tab_gld->Init() || !_tab_boat->Init() || !_tab_res->Init() 
			...
			|| !_tab_amp_setting->Init() || !_tab_amp_team->Init() || !_tab_BossesRespawn->Init())
		return FALSE;
	// bosses respawn tracker
	if (g_Config.IsBossTrackerActive) {
		_tab_BossesRespawn->InitInfoAndSetUp();
	}
	//int issue;
...
bool CGameDB::ReadPlayer(CPlayer *pPlayer, const DWORD atorID)
{
...
bool CGameDB::SavePlayer(CPlayer *pPlayer, const char chSaveType)
{
...
bool CTableGuild::ListAll(CCharacter* pCha , const char disband_days)
{
...
void CTableGuild::TryFor(CCharacter* pCha, const uLong guildid)
{
	if( pCha->HasGuild() )
	{
...
void CTableGuild::TryForConfirm(CCharacter* pCha, const uLong guildid)
{
	char sql[SQL_MAXLEN];
...
bool CTableGuild::GetGuildBank(const uLong guildid, CKitbag * bag){
	string buf[3];
	char filter[80];
...
int CTableGuild::GetGuildLeaderID(const uLong guildid){
	string buf[3];
	char filter[80];
...
bool CTableGuild::UpdateGuildBank(const uLong guildid, CKitbag * bag){
	char sql[SQL_MAXLEN];
...
bool CTableGuild::UpdateGuildBankGold(const int guildID, const int money){
	char sql[SQL_MAXLEN];
...
unsigned long long CTableGuild::GetGuildBankGold(const uLong guildid){
	string buf[1];
	char filter[80];
...
std::vector<CTableGuild::BankLog> CTableGuild::GetGuildLog(const uLong guildid) {
	string buf[1];
...
bool CTableGuild::SetGuildLog(std::vector<BankLog> log, const uLong guild_id) {
	char sql[SQL_MAXLEN];
	char data[8000];
	data[0] = '\0';
...
bool CTableGuild::GetGuildInfo(CCharacter* pCha, const uLong guildid )
{
	string buf[4];
	char filter[80];
...
bool CTableGuild::Approve(CCharacter* pCha, const uLong chaid)
{
	if(!pCha || !pCha->HasGuild())
	{
		return false;
...
bool CTableGuild::Reject(CCharacter* pCha, const uLong chaid)
{
	if(!pCha || !pCha->HasGuild())
	{
		return false;
...
bool CTableGuild::Kick(CCharacter* pCha, const uLong chaid)
{
	if(!pCha || !pCha->HasGuild())
	{
		return false;
...
bool CTableGuild::GetGuildName(const long lGuildID, std::string& strGuildName)
{
	char filter[80];
...
bool CTableGuild::Leizhu( CCharacter* pCha, const BYTE byLevel, const DWORD dwMoney )
{
...
bool CTableGuild::Challenge( CCharacter* pCha, const BYTE byLevel, const DWORD dwMoney )
{
	if(!pCha || !pCha->HasGuild() || byLevel < 1 || byLevel > 3 )
	{
		return false;
...
bool CTableGuild::HasGuildLevel( CCharacter* pChar, const BYTE byLevel )
{
	if( !pChar->HasGuild() )
	{
		return false;
...
bool CTableGuild::HasCall(const BYTE byLevel )
{
	string buf[5];
	char filter[80];
...
bool CTableGuild::StartChall(const BYTE byLevel )
{
	//LG( "��ս����", "���������%d��ս����ʼ����...\n", byLevel ); 
...
void CTableGuild::EndChall(const DWORD dwGuild1, const DWORD dwGuild2, const BOOL bChall )
{
...
bool CTableGuild::ChallWin(const BOOL bUpdate, BYTE byLevel, const DWORD dwWinGuildID, const DWORD dwFailerGuildID )
{
	// ��ʼ����
	if( !begin_tran() )
	{
...
void CTableGuild::ChallMoney(const BYTE byLevel, const BOOL bChall, const DWORD dwGuildID, const DWORD dwChallID, DWORD dwMoney )
{
	if( bChall )
	{
...
bool CTableGuild::GetChallInfo(const BYTE byLevel, DWORD& dwGuildID1, DWORD& dwGuildID2, DWORD& dwMoney )
{
	string buf[3];
	char filter[80];
...
bool CTableCha::SetGuildPermission(const int atorID, const unsigned long permission, const int guild_id)
{
...
bool CTableCha::SetChaAddr(const DWORD atorID, Long addr)
{
...
bool	CTableItem::LockItem(	SItemGrid*	sig, const int	iChaId	)
{
	char param[80];
...
bool CTableItem::UnlockItem( SItemGrid* sig, const int iChaId )
{
	if(sig)
	{
...
//respawn boss tracker @mothannakh
bool CTableBossesRespawn::Init(void)
{
	_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "select \
				   characterId, respawntime, respawnTicks, PoseX, PoseY,Angle,MapName\
				   from %s \
				   (nolock) where 1=2", \
		_get_table());
	printf_s("Loading BossesRespawn \n");
	if (strlen(g_sql) >= SQL_MAXLEN)
	{
		FILE* pf = nullptr;
		if (fopen_s(&pf, "log\\SQLsentence_length_slopover.txt", "a+") != 0) {
			LG("error", " CTableBossesRespawn Load Raw Data Info Bin File [log\\SQLsentence_length_slopover.txt] Failed!\n");
			return FALSE;
		}
		if (pf)
		{
		...
		LG("CTableBossesRespawn", "SQL sentence length slop over\n");
		return false;
	}
	short sExec = exec_sql_direct(g_sql);
	if (!DBOK(sExec))
	{
		char buffer[255];
		_snprintf_s(buffer, sizeof(buffer), _TRUNCATE, RES_STRING(GM_GAMEDB_CPP_00001), "BossesRespawn");
		MessageBox(0, buffer, RES_STRING(GM_GAMEDB_CPP_00002), MB_OK);
		return false;
	}
...
bool CTableBossesRespawn::InsertDBBossInfo(const uInt characterId, const uInt respawntime, const uInt respawnTicks, const uInt PoseX, const uInt PoseY, const uInt Angle, cChar* MapName) {
	T_B
		//fill map info first 
		CGameConfig::BossesInfo BossFullInfo;
	BossFullInfo.characterId = characterId;
	BossFullInfo.respawntime = respawntime; //just fill it as zero will be updated when monster get killed 
	BossFullInfo.respawnTicks = respawnTicks;
	BossFullInfo.PoseX = PoseX;
	BossFullInfo.PoseY = PoseY;
	BossFullInfo.Angle = Angle;
	BossFullInfo.MapName = MapName;
	//insert it to our map data 
	g_Config.BossRespawnInfo.insert_or_assign(characterId, BossFullInfo);
	//
	_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "INSERT INTO %s (characterId, respawntime, respawnTicks, PoseX, PoseY,Angle, MapName) VALUES (%d, %d, %d, %d, %d,%d, '%s')", _get_table(), characterId, respawntime, respawnTicks, PoseX, PoseY, Angle, MapName);

		if (const short sExec = exec_sql_direct(g_sql); !DBOK(sExec) || get_affected_rows() == 0)
	{
		printf_s("failed get_affected_rows InsertDBBossInfo ");
		this->rollback();
		return false;
	}
	...
	if (!commit_tran())
	{
		printf_s("commit_tran failed to InsertDBBossInfo ");
		this->rollback();
		return false;
	}
	return true;
...
//to send to player client // to not use packet to all gameserver will generate data every 1 min
void CTableBossesRespawn::GenerateBossesSpawnTimer(CCharacter* pCha) {
	T_B
		if (!pCha)
			return;
	static DWORD COOLDOWN = GetTickCount();
	static std::map<unsigned int, CGameConfig::BossesInfo> TempBossesInfo{}; // temp info to send info to client 
	//if request came before cd up we use tempdata to send client info 
	if (COOLDOWN > GetTickCount()) {
		//just to make sure we have any bosses to send
		if (TempBossesInfo.empty())
			return;

		//send packet
		WPACKET WtPk = GETWPACKET();
		WRITE_CMD(WtPk, CMD_MC_BossTimer);
		WRITE_SHORT(WtPk, TempBossesInfo.size());
		for (const auto& val : TempBossesInfo | views::values) {
			const auto& respawninfo = val;
			// write rest of bosses info to player client
			// Get monster name
			if (const auto pRec = GetChaRecordInfo(respawninfo.characterId); pRec) {
				WRITE_STRING(WtPk, pRec->szName);
			}
			else
				WRITE_STRING(WtPk, "unknown");
			// write map name
			WRITE_STRING(WtPk, respawninfo.MapName.c_str());
			// send cord location
			std::string location = " at Unknown";
			if (respawninfo.PoseX > 0 && respawninfo.PoseY > 0) {
				location = " at " + to_string(respawninfo.PoseX / 100) + "," + to_string(respawninfo.PoseY / 100);
			}
			// send location
			WRITE_STRING(WtPk, location.c_str());
			const auto ctime = static_cast<unsigned int>(std::time(nullptr));
			// send next respawn
			auto nextrespawn = static_cast<int>(respawninfo.respawnTicks - ctime);
			if (nextrespawn < 0)
				nextrespawn = 0;
			WRITE_LONG(WtPk, nextrespawn);
			std::string states = "Alive";
			// check if monster alive

			// means boss still didn't respawn and dead
			if (respawninfo.respawnTicks > ctime) {
				states = "Dead";
			}
			WRITE_STRING(WtPk, states.c_str());
		}
		// send all info to player client
		pCha->ReflectINFof(pCha, WtPk);
	}
	else {
		_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "SELECT characterId, respawntime, respawnTicks, PoseX, PoseY,Angle, MapName FROM %s  WHERE characterId >0", _get_table());

		if (strlen(g_sql) >= SQL_MAXLEN) {
			FILE* pf = nullptr;
			if (fopen_s(&pf, "log\\SQLsentence_length_slopover.txt", "a+") != 0) {
				LG("error", " GenerateBossesSpawnTimer Load Raw Data Info Bin File [log\\SQLsentence_length_slopover.txt] Failed!\n");
				return;
			}
			if (pf) {
				fprintf(pf, "%s\n\n", g_sql);
			...
			LG("CTableBossesRespawn", " GenerateBossesSpawnTimer SQL sentence length slop over\n");
			return;
		}
		...
		//make sure we have data to be sent 
		if (totalRows <= 0)
			return;
		TempBossesInfo.clear();
		// send info to client
		WPACKET WtPk = GETWPACKET();
		WRITE_CMD(WtPk, CMD_MC_BossTimer);
		WRITE_SHORT(WtPk, totalRows);
		//fetch info from db 
		for (int i = 0; i < totalRows; i++) {
			const auto charid = Str2Int(data[i][0]);
			// check if selected id in our tracker ids list
			// all info we may get from our sql query
			const auto respawntime = Str2Int(data[i][1]);
			const auto respawnTicks = Str2Int(data[i][2]);
			const auto PoseX = Str2Int(data[i][3]);
			const auto PoseY = Str2Int(data[i][4]);
			const auto Angle = Str2Int(data[i][5]);
			const auto& MapName = data[i][6];
			// if ids in our tracker list and exist in DB we fill boss struct info in our map
			CGameConfig::BossesInfo BossFullInfo{};
			BossFullInfo.characterId = charid;
			BossFullInfo.respawntime = respawntime;
			BossFullInfo.respawnTicks = respawnTicks;
			BossFullInfo.PoseX = PoseX;
			BossFullInfo.PoseY = PoseY;
			BossFullInfo.Angle = Angle;
			BossFullInfo.MapName = MapName;
			// insert it to our map data
			// TempBossesInfo.insert(std::make_pair(charid, BossFullInfo));
			TempBossesInfo.insert_or_assign(charid, BossFullInfo);
			// write rest of bosses info to player client
			// Get monster name
			if (const auto pRec = GetChaRecordInfo(charid); pRec) {
				WRITE_STRING(WtPk, pRec->szName);
			}
			else
				WRITE_STRING(WtPk, "unknown");
			//write map name 
			WRITE_STRING(WtPk, MapName.c_str());
			//send cord location 
			std::string location = " at Unknown";
			if (PoseX > 0 && PoseY > 0) {
				location = " at " + to_string(PoseX / 100) + "," + to_string(PoseY / 100);
			}
			//send location 
			WRITE_STRING(WtPk, location.c_str());
			const auto ctime = static_cast<unsigned int>(std::time(nullptr));
			//send next respawn 
			int nextrespawn = static_cast<int>(BossFullInfo.respawnTicks - ctime);
			if (nextrespawn < 0)
				nextrespawn = 0;
			WRITE_LONG(WtPk, nextrespawn);
			std::string states = "Alive";
			...
			//means boss still didn't respawn and dead 
			if (BossFullInfo.respawnTicks > ctime) {
				states = "Dead";
			}
			WRITE_STRING(WtPk, states.c_str());
	...
	COOLDOWN += 50000; // client has cd as 1 min so we keep server on 50 second cd 

	T_E
}
bool CTableBossesRespawn::UpdateDBBossInfo(const uInt characterId, const uInt respawntime, const uInt respawnTicks, const uInt PoseX, const uInt PoseY, const uInt Angle, cChar* MapName) {
	T_B
		_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "Update %s set characterId =%d,respawntime =%d,respawnTicks =%d,PoseX =%d,PoseY =%d,Angle =%d,MapName ='%s' where characterId = %d;", _get_table(), characterId, respawntime, respawnTicks, PoseX, PoseY, Angle, MapName, characterId);
		if (const short sExec = exec_sql_direct(g_sql); !DBOK(sExec) || get_affected_rows() == 0)
	{
		printf_s("failed get_affected_rows UpdateDBBossInfo ");
		this->rollback();
		return false;
	}
	...
	if (!commit_tran())
	{
		printf_s("commit_tran failed to UpdateDBBossInfo ");
		this->rollback();
		return false;
	}
	return true;
...
bool CTableBossesRespawn::EraseBossFromDB(const uInt characterId) {
	T_B
		if (characterId > 0 && g_Config.IsBossTrackerActive) {
			_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "delete from %s where characterId =%d;  declare @max int select @max=ISNULL(max([Id]),0) from [BossesRespawn]; DBCC CHECKIDENT ('[BossesRespawn]', RESEED, @max )", _get_table(), characterId);

			const short sExec = exec_sql_direct(g_sql);
			//if we removed from db remove from gameserver memory ?
			g_Config.BossesRespawnIDS.erase(characterId);
			return DBOK(sExec);
		}
	return false;
	T_E
}
bool CTableBossesRespawn::EraseAllBossesFromDB() {
	T_B
		if (!g_Config.IsBossTrackerActive)
			return false;
	_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "TRUNCATE TABLE %s; declare @max int select @max=ISNULL(max([Id]),0) from [BossesRespawn]; DBCC CHECKIDENT ('[BossesRespawn]', RESEED, @max )", _get_table());
...
bool CTableBossesRespawn::InitInfoAndSetUp() {
	T_B
		_snprintf_s(g_sql, sizeof(g_sql), _TRUNCATE, "SELECT characterId, respawntime, respawnTicks, PoseX, PoseY,Angle, MapName FROM %s  WHERE characterId >0", _get_table());
	...
	if (strlen(g_sql) >= SQL_MAXLEN)
	{
		FILE* pf = nullptr;
		if (fopen_s(&pf, "log\\SQLsentence_length_slopover.txt", "a+") != 0) {
			LG("error", " InitInfoAndSetUp Load Raw Data Info Bin File [log\\SQLsentence_length_slopover.txt] Failed!\n");
			return FALSE;
		}
		if (pf)
		{
		...
		LG("CTableBossesRespawn", " InitInfoAndSetUp SQL sentence length slop over\n");
		return false;
	}
	...
	for (int i = 0; i < totalRows; i++) {
		//check if selected id in our tracker ids list 
		if (const auto charid = Str2Int(data[i][0]); g_Config.BossesRespawnIDS.contains(charid))
		{
			//all info we may get from our sql query 
			const auto respawntime = Str2Int(data[i][1]);
			const auto respawnTicks = Str2Int(data[i][2]);
			...
			BossFullInfo.MapName = MapName;
			//insert it to our map data 
			g_Config.BossRespawnInfo.insert_or_assign(charid, BossFullInfo);
		}
	}
	std::cout << "Setup Bosses Respawn Tracker Total List in DB:" << totalRows << ",total Tracked List " << g_Config.BossRespawnInfo.size()
		<< '\n';
	return true;
	T_E
}

bool CTableBossesRespawn::IsBossExitInDataBase(const uInt characterId) {
	T_B
		std::string buf[1];
	constexpr char param[] = "characterId";
	char filter[80];
	_snprintf_s(filter, sizeof(filter), _TRUNCATE, "characterId =%d", characterId);
	const bool ret = _get_row(buf, 1, param, filter);
	if (const int r1 = get_affected_rows(); ret && r1 > 0) {
		return true;
	}
	return false;
	T_E
