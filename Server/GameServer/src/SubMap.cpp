...
,m_bIsRun(false)
{
	...
	// respawn tracker ticks
	m_BossesPendingRespawn.Begin(15000); // 15 seconds
...
//=============================================================================
CCharacter* SubMap::ChaSpawn(const Long lChaInfoID, Char chCtrlType, const Short sAngle, const Point* pSPos, const bool bEyeshotAbility, cChar* cszChaName, const long clSearchRadius, const long lReliveTime) {
	T_B

		if (!IsValidPos(pSPos->x, pSPos->y))
			return nullptr;

	CChaRecord* pCChaRecord = GetChaRecordInfo(lChaInfoID);
	if (!pCChaRecord)
		return nullptr;
	// start boss tracker system
	// lets make it run once upon start only
	if (g_Config.GameServerFirstRun) {

		if (g_Config.IsBossTrackerActive) {
			// if the respawnned monster in our tracker list , we check if can spawn it or not the time yet
			if (g_Config.BossesRespawnIDS.contains(static_cast<unsigned int>(lChaInfoID))) {
				// check if can rpawn yet
				if (const auto& BossInfo = g_Config.BossRespawnInfo.find(lChaInfoID); BossInfo != g_Config.BossRespawnInfo.end()) {
					if (const auto& BossFullInfo = BossInfo->second; BossFullInfo.characterId == static_cast<unsigned int>(lChaInfoID) && strcmp(BossFullInfo.MapName.c_str(), GetName()) == 0 && BossFullInfo.respawntime > 0) {
						if (const auto ctime = static_cast<unsigned int>(std::time(nullptr)); BossFullInfo.respawnTicks >= ctime) {
							const auto respawnafter = BossFullInfo.respawnTicks - ctime;
							LG("RespawnController", "pending respawn %s at %u,%u %s , respawn after %u Seconds\n", pCChaRecord->szName, BossFullInfo.PoseX, BossFullInfo.PoseY, BossFullInfo.MapName.c_str(), respawnafter);
							// can't spawn so break and add to respawn ticks
							g_Config.BossesShouldRespawn.insert(lChaInfoID);
							return nullptr;
						}
					}
				}
				else {
					// else means not exist even in our database so just fill struct and insert to db for future use
					// fill monster full info
					// INsert to DB + fill the map
					game_db.InsertDBBossInfo(lChaInfoID, lReliveTime, static_cast<unsigned int>(std::time(nullptr)), pSPos->x, pSPos->y, sAngle, GetName());
				}
			}
		}
	}
	CCharacter* pCCha = g_pGameApp->GetNewCharacter();
	if (!pCCha)
		return nullptr;
	...
	pCCha->SetCat(static_cast<short>(pCCha->m_pCChaRecord->lID));
	pCCha->SetID(g_pGameApp->m_Ident.GetID());
	pCCha->m_AIType = static_cast<BYTE>(pCChaRecord->lAiNo);
	pCCha->m_sChaseRange = static_cast<short>(pCChaRecord->lCDis);
...
void SubMap::Run(const DWORD dwCurTime) {
	...
	if (g_Config.IsBossTrackerActive) {
		if (!g_Config.BossesShouldRespawn.empty()) {
			if (m_BossesPendingRespawn.IsOK(dwCurTime)) {
				const auto ctime = static_cast<unsigned int>(std::time(nullptr));
				// search for each monster should respawn in our data info
				for (auto& ids : g_Config.BossesShouldRespawn) {
					// if we have info for boss in our struct
					if (const auto& fullinfo = g_Config.BossRespawnInfo.find(ids); fullinfo != g_Config.BossRespawnInfo.end()) {
						if (const auto& respawninfo = fullinfo->second; respawninfo.respawnTicks < ctime) // means we can spawn it now
						{
							Point Pos{};
							Pos.x = respawninfo.PoseX;
							Pos.y = respawninfo.PoseY;

							if (0 == strcmp(respawninfo.MapName.c_str(), GetName())) {
								if (const auto pCCha = ChaSpawn(ids, enumCHACTRL_NONE, respawninfo.Angle, &Pos); pCCha) {
									pCCha->SetResumeTime(respawninfo.respawntime * 1000);
									g_Config.BossesShouldRespawn.erase(ids);
									LG("RespawnController", "Spawned %s at %ld,%ld %s Successfully\n", pCCha->GetName(), Pos.x, Pos.y, respawninfo.MapName.c_str());
								}
							}
						}
					}
...
void COutMapCha::Run(const unsigned long ulCurTick) {
	T_B

		const long lTickDist = ulCurTick - m_ulTick;
	...
	spawnQueue.erase(
		ranges::remove_if(spawnQueue,
		                  [&](const std::unique_ptr<SMgrUnit>& pSCarrier) {
			                  if (!pSCarrier) {
				                  return true;
			                  }
...
{
	T_B
		// step one monster is dead
		if (pChaInfo->chStep == 1) {
			const auto pChar = pChaInfo->pCCha;
			// respawn tracker @mothannakh
			if (g_Config.IsBossTrackerActive && !pChar->IsPlayerCha()) {
				// check if monster in our bosses respawn control ids
				if (const auto& CharID = pChar->GetCat(); g_Config.BossesRespawnIDS.contains(CharID)) {
					// player pose info
					const auto& pose = pChar->GetPos();
					// boss struct
					const auto RealRespawn = static_cast<unsigned int>(pChar->m_SExistCtrl.lResumeTime / 1000);
					// if monster already exist just update his data
					if (const auto& BossInfo = g_Config.BossRespawnInfo.find(CharID); BossInfo != g_Config.BossRespawnInfo.end()) {
						auto& BossFullInfo = BossInfo->second;
						BossFullInfo.respawntime = RealRespawn;
						BossFullInfo.respawnTicks = static_cast<unsigned int>(std::time(nullptr)) + RealRespawn;
						BossFullInfo.PoseX = pose.x;
						BossFullInfo.PoseY = pose.y;
						BossFullInfo.Angle = pChar->GetAngle();
						BossFullInfo.MapName = pChaInfo->SwitchInfo.szTarMapName;
						// to update db here
						game_db.UpdateDBBossInfo(CharID, RealRespawn, BossFullInfo.respawnTicks, pose.x, pose.y, BossFullInfo.Angle, pChaInfo->SwitchInfo.szTarMapName);

					}
					else {
						// fill monster full info & if not exist add his data to database
						game_db.InsertDBBossInfo(CharID, RealRespawn, static_cast<unsigned int>(std::time(nullptr)) + RealRespawn, pose.x, pose.y, pChar->GetAngle(), pChaInfo->SwitchInfo.szTarMapName);
					}
				}
			}
			// end

			pChaInfo->SwitchInfo.pSrcMap->GoOut(pChar);
			pChar->SetExistState(enumEXISTS_RESUMEING);
			pChaInfo->chStep++;
		}
		else if (pChaInfo->chStep == 2) {
			// step 2 monster in respawn
			switch (pChaInfo->chAction) {
			case enumCHA_TIMEER_ENTERMAP: {
				CCharacter* pCCha = pChaInfo->pCCha;
				if (pCCha->IsPlayerCha()) // What shouldn't happen
				{
					// LG("Monster rebirth error", "Monster name %s, player character name %s(%s)\n", pCCha->GetLogName(), pCCha->GetPlyMainCha()->GetLogName(), pCCha->GetPlyCtrlCha()->GetLogName());
					LG("monster renascence errror", "monster name %s character player name %s(%s)\n", pCCha->GetLogName(), pCCha->GetPlyMainCha()->GetLogName(), pCCha->GetPlyCtrlCha()->GetLogName());
					break;
				}

				// If a non-player character has a life time set
				//rework this little bit if monster has timer and timer finished stop respawn monster otherwise 
				//keep respawn it in normal flow @mothnnakh
				/*
				CheckLifeTime() returns TRUE when time has passed beyond the specified lifetime.
				It returns FALSE when either no lifetime is set or when the elapsed time is still within the lifetime duration.
				*/
				if (pCCha->GetLifeTime() != 0 && !pCCha->canRespawnMonsterLifeTime) {
					break;
				}
				if (pCCha->canRespawnMonsterLifeTime && pCCha->CheckLifeTime()) {
					break;
				}
				...
				pCCha->setAttr(ATTR_HP, pCCha->m_CChaAttr.GetAttr(ATTR_MXHP)); // Current HP
				pCCha->setAttr(ATTR_SP, pCCha->m_CChaAttr.GetAttr(ATTR_MXSP)); // Current SP
				...
				pCCha->ResetAIState(); // Reset the status record required for ai
