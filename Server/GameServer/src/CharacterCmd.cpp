...
// ��maincha���ø�����
bool CCharacter::Cmd_EnterMap(cChar* l_map, const Long lMapCopyNO, uLong l_x, uLong l_y, const Char chLogin)
{T_B
		...
		setAttr(ATTR_SP, 1);
	{
...

		{
			...
			CMapRes* pCMapRes = nullptr;
			SubMap* pCMap = nullptr;
			if (bNewCha) // �½�ɫ
			{
			...
			pCMap = pCMapRes->GetCopy(static_cast<Short>(lMapCopyNO));
			if (!pCMap)
			{
			...
			for (int i = 0; i < enumEQUIP_NUM; i++)
			{
				...
				if (m_SChaPart.SLink[i].sID && (m_SChaPart.SLink[i].expiration - std::time(nullptr)) <= 0 && m_SChaPart.SLink[i].expiration != 0) {
					Short	sUnfixNum = 0;
			...
			const char cAutoLock = m_CKitbag.IsPwdAutoLocked() ? 1 : 0;
			char cLock = m_CKitbag.IsPwdLocked() ? 1 : 0;
			...
			{
				const CLevelRecord* pCLvRec = GetLevelRecordInfo(m_CChaAttr.GetAttr(ATTR_LV));
				const CLevelRecord* pNLvRec = GetLevelRecordInfo(static_cast<long>(m_CChaAttr.GetAttr(ATTR_LV)) + 1);
				if (pCLvRec)
				{
			...
			// ����
			{
				CSailLvRecord* pCLvRec = nullptr, * pNLvRec = nullptr;
				pCLvRec = GetSailLvRecordInfo(m_CChaAttr.GetAttr(ATTR_SAILLV));
				pNLvRec = GetSailLvRecordInfo(static_cast<long>(m_CChaAttr.GetAttr(ATTR_SAILLV)) + 1);
				if (pCLvRec)
				{
			...
			// ����
			{
				CLifeLvRecord* pCLvRec = nullptr, * pNLvRec = nullptr;
				pCLvRec = GetLifeLvRecordInfo(m_CChaAttr.GetAttr(ATTR_LIFELV));
				pNLvRec = GetLifeLvRecordInfo(static_cast<long>(m_CChaAttr.GetAttr(ATTR_LIFELV)) + 1);
				if (pCLvRec)
				{
...
//=============================================================================
void CCharacter::Cmd_BeginMove(const Short sPing, Point *pPath, const Char chPointNum, const Char chStopState)
{T_B
if (!IsLiveing()) {
	return;
}
...
//=============================================================================
void CCharacter::Cmd_BeginSkill(const Short sPing, Point *pPath, const Char chPointNum,
                                CSkillRecord *pSkill, Long lSkillLv, const Long lTarInfo1, const Long lTarInfo2, const Char chStopState)
{T_B
	...
	else
	{
		...
		MoveInit.STargetInfo.ulDist = GetSkillDist(nullptr, pSkill);
...
void CCharacter::Cmd_BeginSkillDirect(const Long lSkillNo, Entity *pTar, const bool bIntelligent)
{T_B
...
void CCharacter::Cmd_BeginSkillDirect2(const Long lSkillNo, const Long lSkillLv, const Long lPosX, const Long lPosY)
{T_B
	CSkillRecord *pSkill = GetSkillRecordInfo(lSkillNo);
	if (pSkill == nullptr)
	{
...
// ʹ�õ���
//=============================================================================
Short CCharacter::Cmd_UseItem(const Short sSrcKbPage, const Short sSrcKbGrid, const Short sTarKbPage, const Short sTarKbGrid)
{T_B
	if (m_CKitbag.IsLock()) // ����������
...
// ʹ��װ�������
//=============================================================================
Short CCharacter::Cmd_UseEquipItem(const Short sKbPage, Short sKbGrid, const bool bRefresh, const bool rightHand)
{T_B
	...
	{
		Short sEquipCon = CanEquipItemNew( sItemId, static_cast<Short>(pSEquipIt->GetFusionItemID()) );
...
// ʹ�����������
//=============================================================================
Short CCharacter::Cmd_UseExpendItem(const Short sKbPage, const Short sKbGrid, const Short sTarKbPage, const Short sTarKbGrid, const bool bRefresh)
{
	T_B
		// @mothannakh
		const DWORD dwCurTime = GetTickCount();
	if (SwitchItemColD > 0 && dwCurTime - SwitchItemColD < 200) {
		// cout << "Tried to use item within less than 200ms of previous item use" << endl;
		return enumITEMOPT_SUCCESS;
	}

	SwitchItemColD = dwCurTime;
	if (!GetActControl(enumACTCONTROL_ITEM_OPT))
		return enumITEMOPT_ERROR_STATE;
	...
	char	szMsg[128];
	//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "���ĵ��ߣ����� %s[%u]������ %u.", pCItemRec->szName, SGridCont.sID, SGridCont.sNum);
...
// 2 ɾ�����ߣ�
//=============================================================================
Short CCharacter::Cmd_UnfixItem(Char chLinkID, Short *psItemNum, const Char chDir, const Long lParam1, Long lParam2, const bool bPriority, const bool bRefresh, const bool bForcible)
{T_B
...
//	}
	...
	if (m_SChaPart.SLink[chLinkID].sID == 0)
		return enumITEMOPT_SUCCESS;
	if (m_SChaPart.SLink[chLinkID].sID == enumEQUIP_BOTH_HAND) // ˫�ֵ���
	{
	...
	const Short	sItemID = m_SChaPart.SLink[chLinkID].sID;
	CItemRecord	*pCItemRec = GetItemRecordInfo(sItemID);
	if (!pCItemRec)
		return enumITEMOPT_ERROR_NONE;
	...
	if (bPriority)// ��ж�����ȼ���λ��
	{
		for (const char i : pCItemRec->szAbleLink)
		{
			const char chAbleL = i;
	...
	if (bRefresh)
	{
		...
		SetLookChangeFlag();
		if (bOptKb)
			m_CKitbag.SetChangeFlag(false, static_cast<Short>(lParam1));
	}
	...
	if (chDir == 1) // �����жװ���������������Ƿ���Է���
	{
		Short sKbPushRet = KbPushItem(false, false, &SUnfixCont, (Short&)lParam2, static_cast<Short>(lParam1));
	...
	if (chDir == 0) // жװ������
	{
		...
		//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "�ӵ��ߣ�ж�����棩������ %s[%u]������ %u.", pCItemRec->szName, SUnfixCont.sID, *psItemNum);
	...
	{
		ChangeItem(false, &m_SChaPart.SLink[chLinkID], chLinkID);
		...
		{
			for (const char i : pCItemRec->szNeedLink)
			{
				const char chNeedL = i;
				if (chNeedL == cchItemRecordKeyValue)
					break;
...
#include "item.h"
//=================================================================================================
Short CCharacter::Cmd_PickupItem(const uLong ulID, const Long lHandle)
{T_B
	if (!GetActControl(enumACTCONTROL_ITEM_OPT))
		return enumITEMOPT_ERROR_STATE;
	...
	if (IsBoat())
		...
		_snprintf_s(szPlyName, sizeof(szPlyName), _TRUNCATE, "%s", GetName());
	char	szMsg[128];
	//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "����ߣ����� %s[%u]������ %u.", pItem->szName, pCItem->m_SGridContent.sID, sPickupNum);
	...
	WRITE_SEQ(WtPk, szTeamMsg, static_cast<uShort>(strlen(szTeamMsg) + 1));
...
Short CCharacter::Cmd_DragItem(Short sSrcGrid, const Short sSrcNum, Short sTarGrid)
{
    if (GetPlyMainCha()->m_CKitbag.IsPwdLocked()) //��������
        return enumITEMOPT_ERROR_KBLOCK;
...
Short CCharacter::Cmd_ThrowItem(const Short sKbPage, const Short sKbGrid, Short *psThrowNum, const Long lPosX, const Long lPosY, const bool bRefresh, const bool bForcible)
{T_B
    if (GetPlyMainCha()->m_CKitbag.IsPwdLocked()) //��������
        return enumITEMOPT_ERROR_KBLOCK;
	...
	*psThrowNum = GridCont.sNum;
	...
	if (IsBoat())
		...
		_snprintf_s(szPlyName, sizeof(szPlyName), _TRUNCATE, "%s", GetName());
	char	szMsg[128];
	//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "�ӵ��ߣ����� %s[%u]������ %u.", pItem->szName, GridCont.sID, GridCont.sNum);
...
// ���ߵĻ�λ���ϲ������.
Short CCharacter::Cmd_ItemSwitchPos(const Short sKbPage, const Short sSrcGrid, const Short sSrcNum, const Short sTarGrid)
{T_B
	if (!GetActControl(enumACTCONTROL_ITEM_OPT))
		return enumITEMOPT_ERROR_STATE;
...
Short CCharacter::Cmd_DelItem(const Short sKbPage, const Short sKbGrid, dbc::Short *psThrowNum, const bool bRefresh, const bool bForcible)
{T_B
	if (!bForcible)
	{	
...
*/
	...
	SItemGrid GridCont;
	GridCont.sNum = *psThrowNum;
	const Short sRet = KbPopItem(bRefresh, bRefresh, &GridCont, sKbGrid, sKbPage); // �����������ɹ�
	if( sRet != enumKBACT_SUCCESS )
		return enumITEMOPT_ERROR_NONE;
	*psThrowNum = GridCont.sNum;
	...
	if (IsBoat())
		...
		_snprintf_s(szPlyName, sizeof(szPlyName), _TRUNCATE, "%s", GetName());
	char	szMsg[128];
	//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "ɾ�����ߣ����� %s[%u]������ %u.", pItem->szName, sItemID, GridCont.sNum);
...
Short CCharacter::Cmd_GuildBankOper(const Char chSrcType, Short sSrcGridID, const Short sSrcNum, const Char chTarType, Short sTarGridID){
...
Short CCharacter::Cmd_BankOper(const Char chSrcType, Short sSrcGridID, const Short sSrcNum, const Char chTarType, Short sTarGridID)
{T_B
	CKitbag	*pCSrcBag, *pCTarBag;
	CCharacter	*pCMainCha = GetPlyMainCha();
	if (chSrcType == 0)
	...
	else
	{
		...
		if (IsBoat())
			...
			_snprintf_s(szPlyName, sizeof(szPlyName), _TRUNCATE, "%s", GetName());
		char	szMsg[128];
		//_snprintf_s(szMsg, sizeof(szMsg), _TRUNCATE, "���в���[%s-->%s]������ %s[%u]������ %u.", chSrcType == 0 ? "������" : "����", chTarType == 0 ? "������" : "����", pItem->szName, sSrcItemID, sLeftNum);
...
{T_B
	...
	{
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BSTR)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BSTR)),
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BDEX)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BDEX)),
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BAGI)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BAGI)),
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BCON)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BCON)),
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BSTA)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BSTA)),
		static_cast<long>(m_CChaAttr.GetAttrMaxVal(ATTR_BLUK)) - static_cast<long>(m_CChaAttr.GetAttr(ATTR_BLUK))
	};
	Long	lAttrPoint = 0;
...
Short CCharacter::Cmd_RemoveItem(const Long lItemID, Long lItemNum, const Char chFromType, const Short sFromID, const Char chToType, Short sToID, const bool bRefresh, const bool bForcible)
{T_B
	...
	if (chFromType == 1 || chFromType == 0) // ��װ�����Ƴ�
	{
		...
		{
			Short	sOptNum = static_cast<Short>(lItemNum);
			sOptRet = Cmd_UnfixItem(static_cast<Char>(sFromID), &sOptNum, chToType, lParam1, lParam2, true, false, bForcible);
		...
		else
		{
			...
			{
				if (m_SChaPart.SLink[i].sID == static_cast<Short>(lItemID))
				{
					sOptNum = static_cast<Short>(lItemNum);
		...
		else
		{
			...
			if (sUseGNum > 0)
			{
				for (Short i = sUseGNum - 1; i >= 0; i--)
				{
					...
					if (m_CKitbag.GetID(sPosID) == static_cast<Short>(lItemID))
					{
						sOptNum = static_cast<Short>(lItemNum);
						if (chToType == 0) // �Ƶ�����
						{
...
// ��ս���󣨵�����������ս��
void CCharacter::Cmd_FightAsk(const dbc::Char chType, const dbc::Long lTarID, const dbc::Long lTarHandle)
{T_B
	...
	WRITE_CHAR(WtPk, static_cast<Char>(getAttr(ATTR_LV)));
	WRITE_STRING(WtPk, g_GetJobName(static_cast<Short>(getAttr(ATTR_JOB))));
	...
	if (chType == enumFIGHT_TEAM)
	{
		...
		while (pCTeamMem = pCPly->GetNextTeamPly())
		{
			...
			WRITE_CHAR(WtPk, static_cast<Char>(pCTeamCha->getAttr(ATTR_LV)));
			WRITE_STRING(WtPk, g_GetJobName(static_cast<Short>(pCTeamCha->getAttr(ATTR_JOB))));
	...
	WRITE_CHAR(WtPk, static_cast<Char>(pCTarCha->getAttr(ATTR_LV)));
	WRITE_STRING(WtPk, g_GetJobName(static_cast<Short>(pCTarCha->getAttr(ATTR_JOB))));
...
{T_B
	...
	Char	chObjNum = static_cast<Char>(pCSrcPly->GetChallengeParam(0));
	...
	// ȷ������
	bool	bHasPly = false;
	Char	chLoop = static_cast<Char>(pCSrcPly->GetChallengeParam(0));
	...
	if (!bFight)
	{
		...
		WPACKET WtPk	= GETWPACKET();
		WRITE_CMD(WtPk, CMD_MC_SYSINFO);
		WRITE_SEQ(WtPk, strNoti.c_str(), static_cast<Short>(strNoti.length()) + 1 );
	...
	if (!pCMCpyCell->HasFreePlyCount(static_cast<Short>(pCSrcPly->GetChallengeParam(0)))) // ��������
	{
	...
	pCTeamFightEntry->SynCopyParam(static_cast<Short>(pCMCpyCell->GetPosID()));
	// ��ʼ����PK��ͼ
	CPlayer	*pCFightMem;
	...
	pCMCpyCell->AddCurPlyNum(static_cast<Short>(lEnterChaNum));
	...
	//
	pCTeamFightEntry->SynCopyRun(static_cast<Short>(pCMCpyCell->GetPosID()), enumMAPCOPY_START_CDT_PLYNUM, lEnterChaNum);

	return;
T_E}

// ������������
void CCharacter::Cmd_ItemRepairAsk(const dbc::Char chPosType, const dbc::Char chPosID)
{T_B
	CPlayer	*pCPly = GetPlayer();
	if (!pCPly)
		return;
...
// ��������Ӧ��
void CCharacter::Cmd_ItemRepairAnswer(const bool bRepair)
{T_B
	CPlayer	*pCPly = GetPlayer();
	if (!pCPly)
		return;
...
// ���߾�������
void CCharacter::Cmd_ItemForgeAsk(const dbc::Char chType, SForgeItem *pSItem)
{T_B
	CPlayer	*pCPly = GetPlayer();
...
void CCharacter::Cmd_ItemLotteryAsk(const SLotteryItem *pSItem)
{T_B
	CPlayer	*pCPly = GetPlayer();
...
void CCharacter::Cmd_LifeSkillItemAsk(const long dwType, SLifeSkillItem *pSItem)
{T_B
	if(m_CKitbag.IsPwdLocked())
	{
		//SystemNotice("���������������ܽ�����ز���.");
	...
	const string& strLifeSkillinfo = GetPlayer()->GetLifeSkillinfo();
...
EndItemForgeAsk:
	...
	WRITE_SHORT(WtPk, static_cast<short>(lOptMoney));
	if(1==dwType)
	{
		string	strVer[2];
...
void CCharacter::Cmd_LifeSkillItemAsR(const long dwType, const SLifeSkillItem *pSItem)
{T_B
	if(m_CKitbag.IsPwdLocked())
	{
		//SystemNotice("���������������ܽ�����ز���.");
	...
	nParamNum++;
	const int nState = lua_pcall(g_pLuaState, nParamNum, LUA_MULTRET, 0);
	if (nState != 0)
	{
	...
	short stime = static_cast<short>(lua_tonumber(g_pLuaState, -2));
	//---------
	//char time[20];
...
{T_B
	...
	const Char sState = m_CKitbag.IsPwdLocked() ? 1 : 0;
...
void CCharacter::Cmd_SetKitbagAutoLock(const Char cAuto)
{T_B
...
// ���߾���Ӧ��
void CCharacter::Cmd_ItemForgeAnswer(const bool bForge)
{T_B
	CPlayer	*pCPly = GetPlayer();
	...
	{
		const CCharacter* pCForgeman = pCPly->GetForgeman();
		if (!pCForgeman)
		{
			goto EndItemForge;
		}
		...
		if (bForge)
		{
			...
			else
			{
				...
				WRITE_CHAR(WtPk, chType);
				WRITE_CHAR(WtPk, static_cast<Char>(lBegin));
...
EndItemForge:
	...
	ForgeAction(false);
T_E}
void CCharacter::Cmd_Garner2_Reorder(const short index)
{
