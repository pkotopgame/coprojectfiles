...
bool CGameConfig::Load(char *pszFileName)
{
	...
	while(!in.eof())
	{
		...
		else if (strKey == "IsBossTrackerActive")
		{
			IsBossTrackerActive = Str2Int(strValue);
		}
		else if (strKey == "BossesRespawn")
		{
			if (IsBossTrackerActive)
			{
				std::string strList[32];
				int nCnt = Util_ResolveTextLine(strValue.c_str(), strList, 32, ',');
				for (int i = 0; i < nCnt; i++)
				{
					auto MonsterID = Str2Int(strList[i]);
					BossesRespawnIDS.insert(MonsterID);
				}
			}
		}
...
bool CGameConfig::Reload(char *pszFileName)
{
	...
	while(!in.eof())
	{
		...
		else if(strKey=="chaos_blind")
		{
			...
			if (IsBossTrackerActive) {
				BossesRespawnIDS.clear(); // clear old list
				std::string strList[32];
				const int nCnt = Util_ResolveTextLine(strValue.c_str(), strList, 32, ',');
				for (int i = 0; i < nCnt; i++) {
					const auto MonsterID = Str2Int(strList[i]);
					BossesRespawnIDS.insert(MonsterID);
				}
			}
		}
