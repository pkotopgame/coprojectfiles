...
//helper for respawn tracker 
int lua_EraseBossFromDB(lua_State* L) {
	const BOOL bValid = (lua_gettop(L) == 1 && lua_isnumber(L, 1));
	if (!bValid) {
		E_LUAPARAM
			return 0;
	}
	const auto GuildId = static_cast<uLong>(lua_tonumber(L, 1));
	if (GuildId < 0) {
		E_LUANULL
			return 0;
	}
	game_db.EraseBossFromDB(GuildId);
	return 0;
}
int lua_EraseAllBossesFromDB(lua_State* L) {
	game_db.EraseAllBossesFromDB();
	return 0;
}

inline int lua_IsBossExitInDataBase(lua_State* L) {
	const BOOL bValid = (lua_gettop(L) == 1 && lua_isnumber(L, 1));
	if (!bValid) {
		E_LUAPARAM
			return 0;
	}
	const auto characterId = static_cast<uLong>(lua_tonumber(L, 1));
	lua_pushboolean(L, game_db.IsBossExitInDataBase(characterId));
	return 1;
}
...
{T_B
	...
	REGFN(EraseBossFromDB)
	REGFN(EraseAllBossesFromDB)
	REGFN(IsBossExitInDataBase)
