...
ToClient::ToClient(char const* fname, ThreadPool* proc, ThreadPool* comm)
	: TcpServerApp(this, proc, comm, false), RPCMGR(this), m_atexit(0), m_calltotal(0), m_maxcon(500), m_lastConnect(0)
{
	const IniFile inf(fname);
	m_maxcon			= std::stoi(inf["ToClient"]["MaxConnection"]); //Modify by lark.li

	m_version		= std::stoi(inf["Main"]["Version"]);
	const IniSection& is = inf["ToClient"];
	const std::string ip = is["IP"];
	const uShort port = std::stoi(is["Port"]);
...
bool ToClient::OnConnect(DataSocket* datasock)
{
	...
	if (g_ddos)
	{
		...
		if (const uLong l_tick = GetCurrentTick(); l_tick - m_lastConnect < 1000 && !strcmp(datasock->GetPeerIP(), const_cast<const char*>(m_lastSockIP)))
		{
...
void ToClient::OnConnected(DataSocket* datasock)
{
	...
	if (g_rsaaes)	// RSA-AES Network Encryption by Mdr - CO 2021
	{
		try
		{
			...
			const CryptoPP::RSA::PublicKey srvPublicKey(l_ply->srvPrivateKey);
			...
			l_wpk.WriteSequence(publickey.data(), publickey.size()); // Send the public key as a const char sequence
			//send if encrypted packet or not
			l_wpk.WriteChar(_comm_enc);
			SendData(datasock, l_wpk);
			printf("\nPublic RSA sent");
...
{
	const uShort	l_cmd		=in_para.ReadCmd();
...
void ToClient::OnProcessData(DataSocket* datasock, RPacket &recvbuf)
{
	...
	try
	{
		...
		if (g_wpe)
		{
			...
			auto buf = const_cast<char*>(recvbuf.GetBuffer());
		...
		if (g_ddos && (l_cmd != CMD_CM_BEGINACTION && l_cmd != CMD_CM_ENDACTION))
		{
			...
			//if (l_ply)
			//{
			if (uLong l_tick = GetCurrentTick(); l_tick - datasock->m_lashTick < 1000 * m_checkSpan)
				{
					++datasock->m_cmdNum;
		...
		case CMD_CM_SEND_PRIVATE_KEY:
		{
			...
			try 
			{
				...
				l_ply->cliPrivateKey.Assign(reinterpret_cast<CryptoPP::byte*>(clientPrivateKey.data()), CryptoPP::AES::MIN_KEYLENGTH);
			}
			catch (std::exception const& e)
			{
		...
		default:		//ȱʡ��ת����GroupServer����GameServer
			...
			{//ת����GameServer
				if (auto l_ply = static_cast<ClientConnection*>(datasock->GetPointer()); l_ply)
				{
			...
			{//ת����GroupServer
				if(auto l_ply = static_cast<ClientConnection*>(datasock->GetPointer()); l_ply)
				{
					...
					{
						if (IniFile inf("GateServer.cfg"); std::stoi(inf["Chaos"]["IsActive"]))
						{
							if (const char* chamap = l_ply->GetMapName(); strcmp(chamap, inf["Chaos"]["Map"].c_str()) == 0)
							{
								WPacket	b_wpk	=datasock->GetWPacket();
								b_wpk.WriteCmd(CMD_MC_SYSINFO);
								auto msg = "Unable to chat in this map!";
								b_wpk.WriteSequence(msg, static_cast<uShort>(strlen(msg))+1);
		...
		{
			if(uLong l_tick		=GetCurrentTick(); l_tick -l_last >1000*30)
			{
				l_last	=l_tick;
...
long TransmitCall::Process()
{
	...
	const uShort l_cmd = m_recvbuf.ReadCmd();
...
{
	try {
		if (const auto l_plyCheck = static_cast<ClientConnection*>(datasock->GetPointer()); l_plyCheck && l_plyCheck->m_actid > 0) {
			WPacket l_wpk = GetWPacket();
		...
		if (const uLong l_tick = GetTickCount() - recvbuf.GetTickCount(); l_ulMilliseconds > l_tick)
		{
			bool bCheat = false;
			...
			const auto l_ply = static_cast<ClientConnection*>(datasock->GetPointer());
			if (!l_ply)//��֯�ظ�����
			{
				return;
			}
			...
			const BYTE byPassword = l_rpk.ReverseReadChar();
...
WPacket ToClient::CM_LOGOUT(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds	=30*1000;
	const uLong	l_tick	=GetTickCount()	-recvbuf.GetTickCount();
	l_ulMilliseconds =(l_ulMilliseconds>l_tick)?l_ulMilliseconds -l_tick:1;

	WPacket	l_retpk	=nullptr;
	...
	if(l_ply)
	{
		...
		try
		{
			...
			}else
			{
				...
				if(const GameServer *l_game	=l_ply->game; (l_ply->m_status	== ClientConnection::Status::Playing) && l_ply->gm_addr && l_game && l_game->m_datasock)
				{
					LogLine l_line(g_gatelog);
					...
					WPacket	sendbuf	=l_game->m_datasock->GetWPacket();
					sendbuf.WriteCmd(CMD_TM_GOOUTMAP);
					sendbuf.WriteChar(0);

					sendbuf.WriteLong(ToAddress(l_ply));
					sendbuf.WriteLong(l_ply->gm_addr);		//��������GameServer�ϵĵ�ַ

					l_ply->game		=nullptr;						//��ֹ����ĵ�GameServer������
					l_ply->gm_addr	=0;						//��ֹ����ĵ�GameServer������

					SendData(l_game->m_datasock,sendbuf);
				}
...
void ToClient::CM_BGNPLAY(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds = 30 * 1000;
	const uLong	l_tick = GetTickCount() - recvbuf.GetTickCount();
	...
	const GameServer* _game_list = g_gtsvr->gm_conn->GetGameList();
	for (const GameServer* t_game = _game_list; t_game; t_game = t_game->next)
	{
...
void ToClient::CM_ENDPLAY(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds	=30*1000;
	const uLong	l_tick	=GetTickCount()	-recvbuf.GetTickCount();
	l_ulMilliseconds =(l_ulMilliseconds>l_tick)?l_ulMilliseconds -l_tick:1;

	if(const auto l_ply = static_cast<ClientConnection*>(datasock->GetPointer()); l_ply)
	{
		auto const l_lockStat = std::lock_guard{l_ply->m_mtxstat};
		...
		{
			if(GameServer *l_game	=l_ply->game; l_game && l_game->m_datasock)
			{				
...
void ToClient::CP_CHANGEPASS(DataSocket* datasock, const RPacket& recvbuf){
	if (const auto l_ply = static_cast<ClientConnection*>(datasock->GetPointer()); l_ply) {
		auto const l_lockStat = std::lock_guard{l_ply->m_mtxstat};
		WPacket l_wpk = WPacket(recvbuf).Duplicate();
...
void ToClient::CM_REGISTER(DataSocket* datasock, const RPacket& recvbuf)
{
	Disconnect(datasock, 100, -25);
	//uLong	l_ulMilliseconds = 30 * 1000;
	//const uLong	l_tick = GetTickCount() - recvbuf.GetTickCount();
...
void ToClient::CM_NEWCHA(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds	=30*1000;
	if(const uLong l_tick	=GetTickCount()	-recvbuf.GetTickCount(); l_ulMilliseconds>l_tick)
	{
...
void ToClient::CM_DELCHA(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds	=30*1000;
	if(const uLong l_tick	=GetTickCount()	-recvbuf.GetTickCount(); l_ulMilliseconds>l_tick)
	{
...
void ToClient::CM_UPDATE_PASSWORD2(DataSocket* datasock, const RPacket& recvbuf)
{
	uLong	l_ulMilliseconds	=30*1000;
	const uLong	l_tick	=GetTickCount()	-recvbuf.GetTickCount();
	if(l_ulMilliseconds>l_tick)
...
bool ToClient::OnEncrypt(DataSocket* datasock, char* ciphertext, uLong ciphertext_len, const char* text, uLong& len)
{
	...
	{
		const auto ply = static_cast<ClientConnection*>(datasock->GetPointer());
...
bool ToClient::OnDecrypt(DataSocket *datasock,char *ciphertext,uLong& len)
{
	...
	if (_comm_enc > 0)
	{
		if (const auto ply = static_cast<ClientConnection*>(datasock->GetPointer()); ply&&ply->handshakeDone) 
		{
...
void ToClient::post_mapcrash_msg(const ClientConnection* ply)
{
	if (ply->m_datasock == nullptr) return;
