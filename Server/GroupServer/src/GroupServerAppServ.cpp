...
char GroupServerApp::SendCharData(Player* player, WPacket& wpk) {
	auto const l_lockDB = std::lock_guard{m_mtxDB};
	std::string l_chaid[Player::emMaxCharacters];
	const char* l_look = const_cast<char*>(m_tblcharaters->GetLook());			//??è?ía1?êy?Y
	LOOK	look; MemSet(reinterpret_cast<char*>(&look), 0, sizeof(LOOK));
	try {
		std::string s(l_look);
		Strin2LookData(&look, s);
		}
	...
	wpk.WriteShort(minimal_look.hairID);
