...
SERVER_NAME = "Pirate King Online"
...
function MsgProc(character, npc, rpk, page, trade, missionlist, exchangeData)
    ...
    elseif usCmd == CMD_CM_FUNCITEM then
		...
		elseif item.func == fullskills1 then
            return fullskills1(character)
...
local PK_Count =	GetChaAttr(role,ATTR_EXTEND6)	
	if PK_Count >= 1 then
		AddChaosPoints(role,-PK_Count)
		SystemNotice(role,"Your Chaos points has been resetted")
	else
		SystemNotice(role,"Sorry, you do not have any Chaos Points.")
	end
end
