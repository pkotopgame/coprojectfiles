...
---------------Determine if the character has some chaos points
...
--[[
		...
		local FightingPoint=GetChaAttr(character,ATTR_CHAOS_POINT)	--GetItemAttr ( ATKER_LD , ITEMATTR_MAXENERGY)
...
--[[
		...
		local DEFER_RYZ1 = GetChaItem2 ( character , 2 , 3849 )
		 HonorPoint = GetItemAttr ( DEFER_RYZ1 , ITEMATTR_VAL_STR)
		]]
		local HonorPoint = GetChaAttr(character,ATTR_CHAOS_POINT)
...
function TakeFightingPoint(character,value)
		...
		local FightingPoint = GetChaAttr(character,ATTR_CHAOS_POINT)
	      --SystemNotice( character , "HasFightingPoint=="..FightingPoint )
	if value > FightingPoint then
		return LUA_FALSE
	else
		...
		AddChaosPoints(character,-value)
		return LUA_TRUE
	end
end
...
function ConditionsTest(character, conditions, param1, param2, npc)
    ...
    if conditions[1] == nil then
        ...
        for i = 1, num, 1 do
			...
			elseif conditions[i].func == NoDailyQuest then
				PRINT("ConditionsTest:NoDailyQuest, p1 = ", conditions[i].p1)
				local ret = NoDailyQuest(character, conditions[i].p1)
				if ret ~= LUA_TRUE then
					PRINT("ConditionsTest:NoDailyQuest = false")
					return LUA_FALSE
				end					
            else
...
function ActionsProc(character, actions, npc, page, misid, scriptid, param1, param2)
    ...
    for i = 1, num, 1 do
		...
		elseif actions[i].func == InstantLV115 then
			PRINT( "ActionsProc:InstantLV115")
			local ret = InstantLV115( character )
			if ret ~= LUA_TRUE then
				PRINT( "ActionsProc:InstantLV115 failed!" )
				return LUA_FALSE
			end
		...
		elseif actions[i].func == SavePlayerQuest then
			PRINT("ActionProc: SavePlayerQuest, p1 = ", actions[i].p1)
			local ret = SavePlayerQuest(character, actions[i].p1)
			if ret ~= LUA_TRUE then
				PRINT("ActionProc:SavePlayerQuest = false")
				return LUA_FALSE
			end				
...
HonorPointCheck = function(Player, Condition, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	end
	local Medal = GetChaItem2(Player, 2, MedalID)
	]]--
	local Honor =  GetChaAttr(Player,ATTR_CHAOS_POINT)	
	if Condition == "<" and Honor < Amount then
		return LUA_TRUE
	elseif Condition == ">" and Honor > Amount then
		return LUA_TRUE
	elseif Condition == "=" and Honor == Amount then
		return LUA_TRUE
	else
...
HonorPointAdd = function(Player, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	...
	SystemNotice(Player, "Increased honor points by: "..Amount..".")
	AddHonor(Player,Amount)
	return LUA_TRUE
end
HonorPointRemove = function(Player, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	...
	local Honor = GetItemAttr(Medal, ITEMATTR_VAL_STR)
	Honor = Honor - Amount
	SetItemAttr(Medal, ITEMATTR_VAL_STR, math.max(Honor, 0))
		SynChaKitbag(Player, 7)
	]]
	SystemNotice(Player, "Decreased honor points by: "..Amount..".")
	AddHonor(Player,-Amount)
	return LUA_TRUE
end
ChaosPointCheck = function(Player, Condition, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	...
	local Chaos = GetChaAttr(Player,ATTR_EXTEND6)
	if Condition == "<" and Chaos < Amount then
		return LUA_TRUE
	elseif Condition == ">" and Chaos > Amount then
		return LUA_TRUE
	elseif Condition == "=" and Chaos == Amount then
		return LUA_TRUE
	else
...
ChaosPointAdd = function(Player, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	...
	local Medal = GetChaItem2(Player, 2, MedalID)
	local Chaos = GetItemAttr(Medal, ITEMATTR_MAXENERGY)
	Chaos = Chaos + Amount
	SetItemAttr(Medal, ITEMATTR_MAXENERGY, Chaos)
	SystemNotice(Player, "Increased chaos points by: "..Amount..".")
	SynChaKitbag(Player, 13)
	]]
	SystemNotice(Player, "Increased chaos points by: "..Amount..".")
	AddChaosPoints(Player,Amount)
	return LUA_TRUE
end
ChaosPointRemove = function(Player, Amount)
--[[
	local MedalID = 3849
	if CheckBagItem(Player, MedalID) < 1 then
		return LUA_FALSE
	...
	local Chaos = GetItemAttr(Medal, ITEMATTR_MAXENERGY)
	Chaos = Chaos - Amount
	SetItemAttr(Medal, ITEMATTR_MAXENERGY, math.max(Chaos, 0))
	SynChaKitbag(Player, 7)
	]]
	SystemNotice(Player, "Decreased chaos points by: "..Amount..".")
	AddChaosPoints(Player,Amount)
	return LUA_TRUE
end