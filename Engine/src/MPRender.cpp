...
MINDPOWER_API float g_fFrameRate = 0.0f;
...
void MPRender::SetCurFrameTick(DWORD dwTick)
{
	if( g_dwCurFrameTick )
	{
		g_fFrameRate = (dwTick - g_dwCurFrameTick) / 30.333333f;
...
void MPRender::EndRender(const bool present) // vim
{
	...
	if (present) {
		...
		[[maybe_unused]] static int g_nCapCnt = 0;
		...
		if (_bCaptureScreen) // 单次截屏
		{
			...
			tm ltime{};
			_gmtime64_s(&ltime, &t);
			_snprintf_s(pszName, _TRUNCATE, "screenshot/%04d-%02d-%02d", ltime.tm_year + 1900, ltime.tm_mon + 1, ltime.tm_mday);
			//Util_MakeDir(pszName);

			char fileName[MAX_PATH];
			_snprintf_s(fileName, _TRUNCATE, "%s-%05d.png", pszName, g_nScreenCap);
...
///////////////////////////////////////////////
bool SurfaceToBMP( IDirect3DSurfaceX* pSurface, const char * strName, int nFlag)
{
	...
	LPBYTE pbtTmp = new BYTE[Desc.Width * Desc.Height * 4];	//new buffer fix for 4k screens "screen shot craash " //mothannakh
...
void	MPRender::CaptureScreen(char*	strFilename)
{
	...
	_pD3DDevice->CreateOffscreenPlainSurface(sDesc.Width,sDesc.Height,sDesc.Format,D3DPOOL_DEFAULT,&pCapSurface,NULL);

	_pD3DDevice->UpdateSurface(pBackBuffer, NULL, pCapSurface, NULL);
