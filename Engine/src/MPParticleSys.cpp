...
//-----------------------------------------------------------------------------
void CMPPartSys::SetEmissionPath(const CEffPath* pcPath)
{
	if(!pcPath)
	{
		_bModelRange = false;
...
void	CMPPartSys::InitParam()
{
	...
	for(int n = 0; n < _wFrameCount; n++)
	{
		...
		auto xc= D3DXVECTOR3(0, 0, 0);
		_vecFrameAngle.push_back(xc);
...
//-----------------------------------------------------------------------------
void CMPPartSys::Reset(const bool	bLife)
{
	_bPlay		= false;
	_fCurTime = 0;
	_bPlay		= false;
	...
	//_vPos		= D3DXVECTOR3( -_fRange[0] / 2, -_fRange[1] / 2,-_fRange[2] / 2);
	auto x1 = D3DXVECTOR3(0, 0, 0);
	MoveTo(&x1);
	for(WORD n = 0; n < _iParNum; ++n)
	{
		_vecParticle[n]->m_bLive = bLife;
		auto xx = D3DXVECTOR3(0, 0, 0);
		_vecParticle[n]->Reset(xx);
...
void _FrameMoveRange(CMPPartSys* pPart, DWORD dwDailTime) {
		...
		if (pPart->_pcPath) {
			if (pPart->_fLife > 1) {
				if (pPart->_pcPath->IsEnd()) {
					pPart->_wDeath++;
					if (pPart->_wDeath >= static_cast<int>(pPart->_fLife)) {
						pPart->_bPlay = false;
						return;
					}
					_CreateRange(pPart, nullptr);
					pPart->_pcPath->Reset();
					return;
				}
			}
			const D3DXVECTOR3* pstart = pPart->_pcPath->GetNextPos();
			const D3DXVECTOR3* pend = pPart->_pcPath->GetCurPos();
	...
	for (WORD n = 0; n < pPart->_iParNum; ++n) {
		CMPParticle* pParticle = pPart->_vecParticle[n];
		if (pParticle->m_bLive) {
...
void _FrameMoveShrink(CMPPartSys* pPart, DWORD dwDailTime) {
	...
	for (WORD n = 0; n < pPart->_iParNum; ++n) {
		CMPParticle* pParticle = pPart->_vecParticle[n];
		if (pParticle->m_bLive) {
			const WORD wCurFrame = pParticle->GetCurFrame(*pPart->_pfDailTime, pPart->_wFrameCount);
			if (wCurFrame == pPart->_wFrameCount) {
				continue;
			}
			if (wCurFrame == pPart->_wFrameCount - 1)
				...
				wNextFrame = wCurFrame + 1;
			const float fLerp = pParticle->GetLerpValue();
			pParticle->m_fSize = *pPart->_vecFrameSize[wCurFrame] +
				(*pPart->_vecFrameSize[wNextFrame] - *pPart->_vecFrameSize[wCurFrame]) * fLerp;
...
//加入路径保存
	...
	if(_bModelRange)
	{
		...
		std::vector<D3DXVECTOR3>::pointer pt = _vecPointRange.data();
...
bool	CMPPartSys::LoadFromFile(FILE* t_pFile, DWORD dwVersion)
{
	...
	if(dwVersion > 11)
	{
		if(_bModelRange)
		{
			...
			_vecPointRange.resize(_wVecNum);
			const std::vector<D3DXVECTOR3>::pointer pt = _vecPointRange.data();
...
//-----------------------------------------------------------------------------
void CMPPartSys::LoadPathFileLet(const char* pszfile)
{
	SAFE_DELETE(_pcPath);
	_pcPath = new CEffPath;
...
//-----------------------------------------------------------------------------
void  CMPPartSys::SetPathVel(float fvel) const
{
	if(_pcPath)
		_pcPath->SetVel(fvel);
}

float CMPPartSys::GetPathVel() const
{
	if(_pcPath)
