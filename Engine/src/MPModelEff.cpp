...
bool	CEffPath::LoadPathFromFile(const char* pszName)
{
	FILE*     stream;
	char      header[4];
	DWORD     version,num;
...
bool CEffPath::LoadPathFromFileLet(const char* file)
{
    ...
    if(LW_FAILED(et.Load(file)))
        return false;
    lwIAnimDataMatrix* data = et.GetData();
    int j = data->GetFrameNum();
    for(i = 0; i < j; i++)
    {
...
void	CEffPath::SavePath(FILE* pf) const
{
...
//!保存效果到文件
bool	CMPModelEff::SaveToFile(char* pszFileName) const
{
	FILE* t_pFile;
