...
{
    LW_RESULT ret = LW_RET_FAILED;
    lwISystem* sys = nullptr;
    lwISysGraphics* sys_graphics = nullptr;
    lwIDeviceObject* dev_obj = nullptr;

    if (LW_FAILED(lwCreateObjectGUID(reinterpret_cast<LW_VOID**>(&sys), LW_GUID_SYSTEM)))
        goto __ret;
    ...
    if (LW_FAILED(sys->GetInterface(reinterpret_cast<LW_VOID**>(&path_info), LW_GUID_PATHINFO)))
        goto __ret;
...
LW_RESULT lwInitMeshLibSystem(lwISystem** ret_sys, lwISysGraphics** ret_sys_graphics, IDirect3DX* d3d, IDirect3DDeviceX* dev, HWND hwnd)
{
        ...
        goto __ret;
    {
        ...
        ssm->CreateStreamEntitySeq(3096, 3096);
        ...
        ssm->CreateIndexBufferStream(3, 8 * 1024 * 1024);
        lwIDynamicStreamMgr* dsm = res_mgr->GetDynamicStreamMgr();
        dsm->Create(256 * 512, 512 * 256);
