...
void MPMap::Render()
{
	...
	{
		_RenderFocusRect();
		D3DXVECTOR3 v[] = { D3DXVECTOR3(_fShowCenterX, _fShowCenterY, 0.0f), D3DXVECTOR3(_fShowCenterX, _fShowCenterY, 4.0f) };
		g_Render.AddLine(v[0], v[1], 0xFF0000FF);
	}
...
void MPMap::RenderSmMap()
{
	...
	g_Render.SetSamplerState(1, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
	...
	g_Render.SetSamplerState(0, D3DSAMP_MIPFILTER, D3DTEXF_POINT);
...
groupend:
	...
	if (bFull) 
		LG("error", "msg The number of map rendering blocks (number of faces>%d) exceeds the maximum value", MAX_RENDER_TILE * 2);

	nLockVertex = 0;

	for (auto& n : TileGroup)
	{
		for (auto& i : n)
		{
			if (STileGroup* pGroup = &i; pGroup->sTileCnt)
			{
