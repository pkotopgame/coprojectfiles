...
: _res_mgr( res_mgr ), _scene_mgr(nullptr), _id( LW_INVALID_INDEX ), _anim_agent( nullptr )
{
    _file_name[ 0 ] = '\0';
...
: _id( LW_INVALID_INDEX ), _anim_agent( nullptr )
{
...
LW_RESULT lwPhysique::GetLinkCtrlMatrix(lwMatrix44* mat, DWORD link_id)
{
    ...
    lwIAnimCtrlObjBone* bone_ctrl = static_cast<lwIAnimCtrlObjBone*>(_anim_agent->GetAnimCtrlObj(&type_info));
    if (bone_ctrl == nullptr)
        goto __ret;
    {
        const lwMatrix44* rtm = bone_ctrl->GetDummyRTM(link_id);
...
LW_RESULT lwPhysique::LoadPrimitive(DWORD part_id, lwIGeomObjInfo* geom_info)
{
    ...
    if( info->anim_size > 0 )
    {
        imp->LoadAnimData( &info->anim_data, tex_path, nullptr );
    }
...
LW_RESULT lwPhysique::HitTestPrimitive( lwPickInfo* info, const lwVector3* org, const lwVector3* ray )
{
    ...
    for( DWORD i = 0; i < LW_MAX_SUBSKIN_NUM; i++ )
    {
        if( (imp_generic = _obj_seq[ i ]) == nullptr )
            continue;
...
void lwPhysique::ShowHelperObject( int show )
{
    ...
    for( DWORD i = 0; i < LW_MAX_SUBSKIN_NUM; i++ )
    {
        if( _obj_seq[ i ] == nullptr )
            continue;

        if((helper_obj = _obj_seq[i]->GetHelperObject()) != nullptr)
        {
