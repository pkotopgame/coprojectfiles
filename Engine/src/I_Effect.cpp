...
bool	CScriptFile::InitAction(const char *ptcsFileName)
{
	...
	while (!feof(fFile))
	{
		...
		StringGetT(tcsTemp, 1023, tcsLine, &lIndex, _TEXT(" ,\x09\x0a\x0d"), 5);
		short iCurType = _ttoi(tcsTemp) - 1;
		if (iCurType < 0)
			continue;
		// 角色的动作数目
...
long StringGetT(_TCHAR *out, long out_max, const _TCHAR *in, long *in_from, const _TCHAR *end_list, long end_len)
{
...
void StringSkipCompartmentT(const _TCHAR *in, long *in_from, const _TCHAR *skip_list, const long skip_len)
{
...
bool	CScriptFile::GetCharAction(const int iCharType, SChaAction *SCharAct)
{
