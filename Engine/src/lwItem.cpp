...
LW_RESULT lwItem::Load(lwIGeomObjInfo* info) {
        ...
        goto __ret;
    {
        ...
        const auto imp = LW_NEW(lwPrimitive(_res_mgr));

        if (LW_FAILED(imp->Load(info, path_info->GetPath(PATH_TYPE_TEXTURE_ITEM), nullptr)))
            goto __ret;
...
LW_RESULT lwItem::Load( const char* file, int arbitrary_flag )
{
    ...
    _res_mgr->QueryObject( reinterpret_cast<void**>(&obj_model), OBJ_TYPE_ITEM, file );
...
LW_RESULT lwItem::GetDummyMatrix(lwMatrix44* mat, DWORD id) {
        ...
        goto __addr_obj_dummy;
    {
        ...
        if (ctrl_obj->GetAnimCtrl() == nullptr)
            goto __addr_obj_dummy;
