...
LW_RESULT lwModel::Load(lwIModelObjInfo* info) {
    ...
    {
        const auto m_info = dynamic_cast<lwModelObjInfo*>(info);
        ...
        const lwIOptionMgr* opt_mgr = sys->GetOptionMgr();
        const BYTE create_helper_primitive = opt_mgr->GetByteFlag(OPTION_FLAG_CREATEHELPERPRIMITIVE);

        for (DWORD i = 0; i < m_info->geom_obj_num; i++) {
            lwIPrimitive* imp;
            _res_mgr->CreatePrimitive(&imp);

            imp->Load(m_info->geom_obj_seq[i], path_info->GetPath(PATH_TYPE_TEXTURE_SCENE), nullptr);
        ...
        if (m_info->helper_data.type != HELPER_TYPE_INVALID) {
            if (_helper_object == nullptr) {
                _helper_object = LW_NEW(lwHelperObject(dynamic_cast<lwResourceMgr*>(_res_mgr)));
...
LW_RESULT lwModel::RenderPrimitive(DWORD id)
{
    ...
    if(lwIPrimitive* p = _obj_seq[id]; _scene_mgr && p->GetState(STATE_TRANSPARENT))
    {
