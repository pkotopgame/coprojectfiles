...
//#endif
	...
	for(int n = 0; n < _iVerNum; n++)
	{
		tv.x = _SShadeUV[n].x;
		tv.y = _SShadeUV[n].y;
		_pModel->m_pDev->SetVertexShaderConstantF(nIndex++, /*_SShadeUV[n]*/tv, 1);
		tv.x = _SShadePos[n].x;
		tv.y = _SShadePos[n].y;
		tv.z = _SShadePos[n].z;
		tv.w = 1;

		_pModel->m_pDev->SetVertexShaderConstantF(nIndex++, /*D3DXVECTOR4(_SShadePos[n].x
...
void	CMPShadeMap::RenderSoft()
{
	...
	_pModel->RenderModel();
	_pModel->End();
	_pModel->m_pDev->SetVertexShader(NULL);
