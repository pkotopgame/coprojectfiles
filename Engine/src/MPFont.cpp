...
#endif
{
	...
	SIZE size;
	::GetTextExtentPoint(_hDc, "ºÃ", 1, &size);
...
void CMPFont::DrawTextClipOnce(char* szText, int nLen, LPRECT psrc, LPRECT pclip, D3DXCOLOR color)
{
	...
	for (int n = 0; n < nLen; n++)
	{
...

		{
			...
			{
				sx += offset;
				continue;//Èç¹ûÕâ¸ö×Ö²»ÔÚ²ÃÇÐÇø£¬²»ÏÔÊ¾
			}
	...
	for (int n = 0; n < nLen; n++)
	{
...

		{
			...
			if (!GetIntersectRect(&dest, &rc, pclip))
			{
				...
				&& rc.top >= pclip->top && rc.bottom <= pclip->bottom)
			{
				_pDev->SetVertexShaderConstantF(9, D3DXVECTOR4(sx, sy, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(10, D3DXVECTOR4(tx1, ty2, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(11, D3DXVECTOR4(sx, sy + h, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(12, D3DXVECTOR4(tx1, ty1, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(13, D3DXVECTOR4(sx + w, sy + h, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(14, D3DXVECTOR4(tx2, ty1, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(15, D3DXVECTOR4(sx + w, sy, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(16, D3DXVECTOR4(tx2, ty2, 0, 1), 1);
				//Èç¹ûÕâ¸ö×ÖÈ«²¿ÔÚ²ÃÇÐÇø£¬ÏÔÊ¾
			}
			else
			{
				...
				_pDev->SetVertexShaderConstantF(9, D3DXVECTOR4(tx, ty, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(10, D3DXVECTOR4(tu1, tv2, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(11, D3DXVECTOR4(tx, (ty + th), 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(12, D3DXVECTOR4(tu1, tv1, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(13, D3DXVECTOR4(tx + tw, (ty + th), 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(14, D3DXVECTOR4(tu2, tv1, 0, 1), 1);
				_pDev->SetVertexShaderConstantF(15, D3DXVECTOR4(tx + tw, ty, 0.9f, 1), 1);
				_pDev->SetVertexShaderConstantF(16, D3DXVECTOR4(tu2, tv2, 0, 1), 1);
...
bool CMPFont::DrawTextShadow(char* szText, int x1, int y1, int x2, int y2,
	D3DXCOLOR color1, D3DXCOLOR color2, bool isOutline) {
	if (!szText)
		return false;
	Begin();
	if (isOutline) {
...
bool CMPFont::Draw3DText(char* szText, D3DXVECTOR3& vPos, D3DXCOLOR color, float fScale)
{
	...
	for (int n = 0; n < nLen; n++)
	{
		...
		if (!_bUseSoft)
		{
			_pDev->SetVertexShaderConstantF(9, D3DXVECTOR4(sx, sy, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(10, D3DXVECTOR4(tx1, ty2, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(11, D3DXVECTOR4(sx, sy + h, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(12, D3DXVECTOR4(tx1, ty1, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(13, D3DXVECTOR4(sx + w, sy, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(14, D3DXVECTOR4(tx2, ty2, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(15, D3DXVECTOR4(sx + w, sy + h, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(16, D3DXVECTOR4(tx2, ty1, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(17, D3DXVECTOR4(sx + w, sy, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(18, D3DXVECTOR4(tx2, ty2, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(19, D3DXVECTOR4(sx, sy + h, 0, 1), 1);
			_pDev->SetVertexShaderConstantF(20, D3DXVECTOR4(tx1, ty1, 0, 1), 1);
		}
		else
		{
#ifdef USE_RENDER
