        ...
        if (!mPixelShaderName.empty())
        {
            ...
            if (mPixelShaderReload)
            {
                ...
                HRESULT hr = D3DXAssembleShaderFromFile(
                    ...
                    NULL, NULL, 0, &codeBuffer, &errorBuffer);    // assemble shader code
                if (SUCCEEDED(hr))
                {
                    hr = device->CreatePixelShader((DWORD*)codeBuffer->GetBufferPointer(), (IDirect3DPixelShaderX**)&mPixelShader);
                }
                else
                {
                    mPixelShader = 0;
...
LW_RESULT lwxRenderCtrlVSVertexBlend::BeginSet(lwIRenderCtrlAgent* agent)
{
    ...
    for (DWORD i = 0; i < animobj_num; i++)
    {
        ...
        if (info.type == ANIM_CTRL_TYPE_BONE)
        {
            ...
            if (FAILED(_const_tab->SetMatrixArray(dev, _const_tab->GetConstantByName(NULL, "mat_bonepallette"), (D3DXMATRIX*)bone_ctrl->GetBoneRTMSeq(), reg_num)))
                return ret;
