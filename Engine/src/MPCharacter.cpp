...
LW_RESULT MPCharacter::GetObjDummyRunTimeMatrix(lwMatrix44* mat, DWORD obj_id, DWORD dummy_id) const
{
    LW_RESULT ret = LW_RET_FAILED;
...
LW_RESULT MPCharacter::GetObjDummyRunTimeMatrix(lwMatrix44* mat, DWORD dummy_id) const
{
    LW_RESULT ret = LW_RET_FAILED;
...
void MPCharacter::SetMaterial(const D3DMATERIALX* mtl) const
{
    lwPhysiqueSetMaterial(_physique, (lwMaterial*)mtl);
}
void MPCharacter::SetOpacity(float opacity) const
{
    _physique->SetOpacity(opacity);
}
void MPCharacter::SetTextureLOD(DWORD level) const
{
    _physique->SetTextureLOD(level);
}
float MPCharacter::GetOpacity() const
{
