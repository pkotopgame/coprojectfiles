...
public:
	...
	BOOL SetTechnique(int iIdx) const;

	//BOOL CreateVertexShader();
	//BOOL SetVertexShader();

	BOOL SetTexture(LPCSTR TextureValue, IDirect3DTextureX* pTexture) const;
	static BOOL SetDword(LPCSTR DwName, DWORD dwvalue);

	BOOL Begin(DWORD  dwIsSave = 0);
	BOOL Pass(UINT ipass);
	BOOL End();

	int	 GetTechCount() const { return _iTechNum;}
	int  GetPassCount() const { return m_passes;}
	void free();
...
inline BOOL CMPEffectFile::SetTechnique(int iIdx) const
{
	//if(iIdx >= _iTechNum)
	//	return false;
...
//	return  TRUE;
//}
inline BOOL CMPEffectFile::SetTexture(LPCSTR TextureValue, IDirect3DTextureX* pTexture) const
{
