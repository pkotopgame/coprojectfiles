...
public:
	...
	static void FillDefaultUV(CEffectModel*	pCModel,TEXCOORD& coord);
...
public:
	...
	bool				LoadPathFromFile(const char* pszName);
    bool                LoadPathFromFileLet(const char* file);
	void				SavePath(FILE* pf) const;
...
public:
	...
	//!保存效果到文件
	bool		 SaveToFile(char* pszFileName) const;
	//!装入效果从文件
	static bool		 LoadFromFile(char* pszFileName);
