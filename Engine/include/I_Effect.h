...
void StringSkipCompartmentT(const _TCHAR *in, long *in_from, const _TCHAR *skip_list, long skip_len);
...
public:
	...
	void GetRotaLoopMatrix(D3DXMATRIX* pmat, float& pCurRota, float fTime) const
	{
		pCurRota += _vRotaLoop.w * fTime;
		if(pCurRota >= 6.283185f)
			pCurRota = pCurRota - 6.283185f;
		const auto value = D3DXVECTOR3(_vRotaLoop.x, _vRotaLoop.y, _vRotaLoop.z);
