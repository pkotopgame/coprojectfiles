...
/*
	...
	\param1		3DÉè±¸
	\param2		×ÖÌåÃû³Æ
	\param3		×ÖÌå´óÐ¡£¨¿ÉÒÔÊ¹ÓÃ16»ò24µãÕó£©
	\param4		ÎÆÀí´óÐ¡µÈ¼¶£¨¿ÉÒÔÎª1 -- 5.·Ö±ðÎª1=64 2=128 3=256 4=512 5=1024¡£
				NOTE:
				1.Èç¹ûÊ¹ÓÃÐ¡ÓÚ256µÄÎÆÀíÀïÊ±£¬ÔÚÊ¹ÓÃÖ£ÂëÊäÈë·¨Ê±ÓÐÊ±Ò»´ÎÊäÈëµÄÎÄ×ÖÊý»á´óÓÚ»º³åÊýÄ¿£¬
				ËùÒÔÎÒÉèÄ¬ÈÏÎÆÀí´óÐ¡Îª256.¶ÔÓÚ¾ø´ó¶àÊýÏÔ¿¨À´Ëµ´´½¨Ò»¸ö256µÄÎÆÀíÓ¦¸Ã²»ÊÇÎÊÌâ°É£¿:)
				2.¾¡Á¿Ê¹ÓÃ16µãÕó´óÐ¡µÄ×ÖÌå,Èç¹ûÒªÊ¹ÓÃ´ó×ÖÌå¿ÉÒÔ¸Ä±äËõ·ÅÖµ
				3.²»Í¬µÄÎÆÀí´óÐ¡Ò»´Î¿ÉÒÔäÖÈ¾µÄÎÄ×ÖÊýÁ¿Ò²ÊÇ²»Ò»ÑùµÄ£¬64´óÐ¡µÄÎÆÀí¿ÉÒÔÒ»´ÎäÖÈ¾µÄÎÄ×ÖÊýÁ¿ÊÇ16¸ö£¬
				128´óÐ¡µÄÊÇ64¸ö£¬256µÄÊÇ256¸ö,512´óÐ¡ÊÇ1024,1024´óÐ¡ÊÇ4096,ÎÆÀíÔ½Ð¡£¬ËÙ¶ÈÔ½¿ì¡£ÒÔÉÏËùËµµÄ
				¶¼ÊÇ16µãÕó´óÐ¡µÄ×ÖÌå£¬Èç¹ûÊÇ24µãÕóµÄ»¹ÒªÉÙÒ»Ð©£©


	bool DrawText( char* szText, int x, int y, D3DCOLOR color = 0xFFFFFFFF,float fScale = 1.0f );
	\param1		ÒªäÖÈ¾µÄÎÄ×Ö
				£¨NOTE:ËùäÖÈ¾µÄÎÄ×ÖµÄ¸öÊý²»ÄÜ´óÓÚ»º³åÊýÄ¿£©
	\param2		ÆÁÄ»ÉÏµÄX×ø±ê
	\param3		ÆÁÄ»ÉÏµÄY×ø±ê
	\param4		ÎÄ×ÖµÄÑÕÉ«
	\param5		¿ÉÒÔ¶ÔÎÄ×Ö´óÐ¡½øÐÐËõ·Å
	return		³É¹¦·µ»Øtrue;
 */
 //===================================================================
 //!¶¨Òå×ÖÌåµÄfvf
...
public:
	...
	struct FONT_VER
	{
		...
		DWORD m_color;
		D3DXVECTOR2 m_fUV;
		void SetValue(const D3DXVECTOR4& _vPos, const DWORD _color, const D3DXVECTOR2& _fUV) {
			constexpr float offs = -0.5f;
			m_vPos = _vPos;
	...
	bool DrawTextShadow(char* szText, int x1, int y1, int x2, int y2,
		D3DXCOLOR color1, D3DXCOLOR color2, bool isOutline = false);
