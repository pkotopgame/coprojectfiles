...
void CBlackTradeMgr::SetItem(stBlackTrade* pBlackTrade) {
	...
	if (pInfo) {
		char szBuffer[128] = { 0 };
		sprintf(szBuffer, "%dx %s", pBlackTrade->sSrcNum, pInfo->szName);
		pItem->SetOwnDefText(szBuffer);
		pItem->SetFullIconPath(pInfo->GetIconFile());
	}
...
void CBlackTradeMgr::ExchangeAnswerProc(bool bSuccess, stBlackTrade* pBlackTrade) {
	if (bSuccess) {
		...
		if (nCurNum < g_stUIBlackTrade.grdItemBuy->GetMaxNum()) {
			...
			if (pInfo) {
				char szBuffer[128] = { 0 };
				sprintf(szBuffer, g_oLangRec.GetString(843), pBlackTrade->sSrcNum, pInfo->szName);
				pItem->SetOwnDefText(szBuffer);
				pItem->SetFullIconPath(pInfo->GetIconFile());
			}
