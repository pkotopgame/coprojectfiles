...
CMaskData* CMaskData::g_MaskData = nullptr;

HRESULT _DbgOuts( TCHAR* strFile, const DWORD dwLine, const HRESULT hr, const TCHAR* strMsg )
{
	TCHAR buffer[256];
...
BOOL	PosInRange(const D3DXVECTOR3& pos, const D3DXVECTOR3& Org, const float fRange)
{
...
: _pCurSuf(nullptr), _pDSSuf(nullptr),_pRenderSurface(nullptr)
{
}
...
bool		CSMallWnd::Create(IDirect3DDeviceX* pDev, const RECT& rcwnd, CGameScene* pScene, const int bksize)
{
	m_pDev = pDev;
	_rcWnd = rcwnd;
...
void		CSMallWnd::MoveTo(const D3DXVECTOR3 &vPos,D3DXVECTOR3&	vCameraDir, const float fCameraAngle,float fCameradDist, const int iAngle)
{
...
void	CSMallMap2D::InitScene()
{
	...
	mi.vertex_seq[0] = MPVector3(static_cast<float>(_rcWnd.left), static_cast<float>(_rcWnd.top), 0.9f);
	mi.vertex_seq[1] = MPVector3(static_cast<float>(_rcWnd.right), static_cast<float>(_rcWnd.top), 0.9f);
	mi.vertex_seq[2] = MPVector3(static_cast<float>(_rcWnd.right), static_cast<float>(_rcWnd.bottom), 0.9f);
	mi.vertex_seq[3] = MPVector3(static_cast<float>(_rcWnd.left), static_cast<float>(_rcWnd.bottom), 0.9f);
...
#ifdef MGR
	//MPIResourceMgr* res_mgr = g_Render.GetInterfaceMgr()->res_mgr;
	if (FAILED(lwLoadTex(&_pTexMask, res_mgr, "texture\\minimap\\mapmask.bmp", nullptr, D3DFMT_A4R4G4B4)))
	{
		LG("ERROR", "msgmapmask.bmp");
	}
	if (FAILED(lwLoadTex(&_pTexDefault, res_mgr, "texture\\minimap\\mapsea.bmp", nullptr, D3DFMT_A4R4G4B4)))
	{
...
void	CSMallMap2D::RenderScene()
{
	...
	{
		const auto val = (_matView * _matProj);
		D3DXMATRIX vpPose;
		D3DXMatrixTranspose(&vpPose, &val);
	...
	int sx = static_cast<int>(_vEyePt.x / SHOWRSIZE);
	int sy = static_cast<int>(_vEyePt.y / SHOWRSIZE);
	if (sx > 0) sx -= 1;
	if (sy > 0) sy -= 1;
	...
	for (m = 0; m < 3; m++)
	{
		for (int n = 0; n < 3; n++)
		{
			...
			else
			{
				if ((_pTex[n][m].x != _sx + n) || (_pTex[n][m].y != _sy + m))
				{
					...
					lwLoadTex(&_pTex[n][m].pTex, g_Render.GetInterfaceMgr()->res_mgr,
						filename, nullptr, D3DFMT_A4R4G4B4);
#else
...
#endif
				(*pVertices++).m_vPos = D3DXVECTOR4(static_cast<float>(sx), static_cast<float>(sy), 0, 1);
				(*pVertices++).m_vPos = D3DXVECTOR4(static_cast<float>(sx) + SHOWRSIZE, static_cast<float>(sy), 0, 1);
				(*pVertices++).m_vPos = D3DXVECTOR4(static_cast<float>(sx) + SHOWRSIZE, static_cast<float>(sy) + SHOWRSIZE, 0, 1);
				(*pVertices++).m_vPos = D3DXVECTOR4(static_cast<float>(sx), static_cast<float>(sy) + SHOWRSIZE, 0, 1);
#ifdef MGR
				_lpSVB->Unlock();
#else
				_pVB->Unlock();
			...
			{
				g_Render.SetVertexShaderConstantF(9, D3DXVECTOR4(static_cast<float>(sx), static_cast<float>(sy),
					0, 1), 1);
				g_Render.SetVertexShaderConstantF(10, D3DXVECTOR4(static_cast<float>(sx) + SHOWRSIZE, static_cast<float>(sy),
					0, 1), 1);

				g_Render.SetVertexShaderConstantF(11, D3DXVECTOR4(static_cast<float>(sx) + SHOWRSIZE, static_cast<float>(sy) +
					SHOWRSIZE, 0, 1), 1);
				g_Render.SetVertexShaderConstantF(12, D3DXVECTOR4(static_cast<float>(sx), static_cast<float>(sy) + SHOWRSIZE,
					0, 1), 1);
			}
#ifdef MGR
			_pVB->DrawSubset(0);
	...
	if (!ResMgr.m_bUseSoft)
	{
		...
		for (m = 0; m < m_pScene->GetSceneObjCnt(); m++)
		{
			...
			if (pscobj /*&& pscobj->IsValid()*/ && pInfo->nFlag != 0)
			{
				...
				{
					_Cha.setTexture(15);
					_Cha.setAngle(static_cast<int>(D3DX_PI));
...
void	CAniWnd::Play(const DWORD dwPlayTime)
{
	if(_bUpdate)
		return;
	_bUpdate = true;
...
void	CAniWnd::ResetTime(const DWORD	dwTime)
{
	_fPlayTime = static_cast<float>(dwTime)/1000;
...
void	CAniWnd::Render()
{
	if (_bUpdate)
	{
		IDirect3DSurfaceX* pSaveSuf = nullptr;
		//IDirect3DSurfaceX*	dsface = NULL;
		IDirect3DSurfaceX* surface = nullptr;
...
void	CAniWnd::MoveWnd(const int x, const int y)
{
	...
	D3DXVECTOR4 v[] = {
		{static_cast<float>(_rcWnd.left),static_cast<float>(_rcWnd.top),0.9f,1},
		{static_cast<float>(_rcWnd.right),static_cast<float>(_rcWnd.top),0.9f,1},
		{static_cast<float>(_rcWnd.right),static_cast<float>(_rcWnd.bottom),0.9f,1},
		{static_cast<float>(_rcWnd.left),static_cast<float>(_rcWnd.bottom),0.9f,1},
		{static_cast<float>(_rcWnd.left),static_cast<float>(_rcWnd.top),0.9f,1},
		{static_cast<float>(_rcWnd.right),static_cast<float>(_rcWnd.top),0.9f,1},
		{static_cast<float>(_rcWnd.right),static_cast<float>(_rcWnd.bottom),0.9f,1},
		{static_cast<float>(_rcWnd.left),static_cast<float>(_rcWnd.bottom),0.9f,1}
...
{
	g_Render.GetDevice()->Clear(0, nullptr, D3DCLEAR_TARGET, 0x00000000, 0, 0);
	D3DXMATRIX matIdentity;
	D3DXMatrixIdentity(&matIdentity);
	g_Render.SetRenderState(D3DRS_ZENABLE, FALSE);
	g_Render.SetRenderState(D3DRS_ZWRITEENABLE, FALSE);
...
void	CCharacter2D::Create(const RECT& rc)
{
	/*
	D3DXVECTOR2 vUV[4] = {
		D3DXVECTOR2(0,0),
...
void	CCharacter2D::MoveWnd(const RECT& rc)
{
	_vp.X = rc.left;
	_vp.Y = rc.top;
...
void	CCharacter2D::Render() 
{
	...
	g_Render.GetDevice()->Clear(0,nullptr,D3DCLEAR_ZBUFFER,0,1,0);
...
void	CCharacter2D::LoadCha(const DWORD dwID, const bool IsMonster) {
	if(!_pModel){
	...
	if( pInfo->chModalType == enumMODAL_MAIN_CHA )
	{
		DWORD part_buf[5] =
		{
			static_cast<DWORD>(pInfo->sSkinInfo[0]),
				static_cast<DWORD>(pInfo->sSkinInfo[1]),
				static_cast<DWORD>(pInfo->sSkinInfo[2]),
				static_cast<DWORD>(pInfo->sSkinInfo[3]),
				static_cast<DWORD>(pInfo->sSkinInfo[4]),
		};
	...
	*/
	...
	D3DXMATRIX	mat;
	D3DXMatrixRotationZ(&mat,static_cast<float>(pChaAttr->sTeamAngle) * 0.01745329f);
...
void CCharacter2D::UpdataFace(const stNetTeamChaPart& stPart, const bool IsPlayer)
{
...
void	CBigMap::Render()
{
	...
	vVer[0].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.top), 0.9f, 1);
	vVer[1].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.top), 0.9f, 1);
	vVer[2].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.bottom), 0.9f, 1);
	vVer[3].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.bottom), 0.9f, 1);
...
Ctemp::Ctemp()
{
	...
	_vWndVer[0].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.top), 0.9f, 1);
	_vWndVer[1].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.top), 0.9f, 1);
	_vWndVer[2].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.bottom), 0.9f, 1);
	_vWndVer[3].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.bottom), 0.9f, 1);
...
void	Ctemp::Render()
{
	if (!_pTex)
	{
		...
		lwLoadTex(&_pTex, g_Render.GetInterfaceMgr()->res_mgr,
			"texture\\ui\\minimap\\mapmask.bmp", nullptr, D3DFMT_A4R4G4B4);
	}
...
void	CMinimap::InitScene()
{
	...
	//	LG("ERROR","msgmapmask.bmp");
	//}
	if (FAILED(lwLoadTex(&_pTexDefault, res_mgr, "texture\\ui\\minimap\\mapsea.bmp", nullptr, D3DFMT_A4R4G4B4)))
	{
		LG("ERROR", "msgmapsea.bmp");
	}
	...
	rc.right = 64;
	rc.bottom = 64;
	_vPicVer[0].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.top), 0.9f, 1);
	_vPicVer[1].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.top), 0.9f, 1);
	_vPicVer[2].m_vPos = D3DXVECTOR4(static_cast<float>(rc.right), static_cast<float>(rc.bottom), 0.9f, 1);
	_vPicVer[3].m_vPos = D3DXVECTOR4(static_cast<float>(rc.left), static_cast<float>(rc.bottom), 0.9f, 1);
...
void	CMinimap::RenderScene()
{
	...
	int tempx = static_cast<int>((-32 * 100 - sxue) / 100 * 1.6f);
	int tempy = static_cast<int>((-32 * 100 - syue) / 100 * 1.6f);
	...
	for (m = 0; m < VIEWRANGE; m++)
	{
		...
		for (int n = 0; n < VIEWRANGE; n++)
		{
			...
			_vPicVer[0].m_vPos = D3DXVECTOR4(static_cast<float>(rct.left), static_cast<float>(rct.top), 0.9f, 1);
			_vPicVer[1].m_vPos = D3DXVECTOR4(static_cast<float>(rct.right), static_cast<float>(rct.top), 0.9f, 1);
			_vPicVer[2].m_vPos = D3DXVECTOR4(static_cast<float>(rct.right), static_cast<float>(rct.bottom), 0.9f, 1);
			_vPicVer[3].m_vPos = D3DXVECTOR4(static_cast<float>(rct.left), static_cast<float>(rct.bottom), 0.9f, 1);
	...
	//////////////////////////////////////////////////////////////////////////
	_vCameraTar[0] = D3DXVECTOR4(static_cast<float>(lenw + _rcWnd.left), static_cast<float>(lenw + 24 + _rcWnd.top), 0, 1);

	g_Render.SetTexture(0, nullptr);
	D3DXMATRIX matIden;
	auto valx = D3DXVECTOR3(static_cast<float>(lenw + _rcWnd.left), static_cast<float>(lenw + _rcWnd.top), 0);
	auto valx2 = D3DXVECTOR3(0, 0, 1);
	GetMatrixRotation(&matIden, &valx, \
		& valx2, _fCameraAngle);
	D3DXVec4Transform(&_vCameraTar[0], &_vCameraTar[0], &matIden);

	_vCameraTar[1] = D3DXVECTOR4(static_cast<float>(lenw / 2 + 6 + _rcWnd.left), static_cast<float>(lenw / 2 + 6 + _rcWnd.top), 0, 1);
	...
	_vCameraTar[1] = D3DXVECTOR4(static_cast<float>(lenw + 26 + _rcWnd.left), static_cast<float>(lenw / 2 + 6 + _rcWnd.top), 0, 1);
	D3DXVec4Transform(&_vCameraTar[1], &_vCameraTar[1], &matIden);
...
void	CLargerMap::Show(const bool bShow)
{
	if(bShow == _bUpdate)
		return;
...
void	CLargerMap::Update( int x, int y )
{
	...
	const float sxue = static_cast<float>(tnx - sx * (SHOWRSIZE * 100))  / static_cast<float>(SHOWRSIZE * 100);
	const float syue = static_cast<float>(tny - sy * (SHOWRSIZE * 100))  / static_cast<float>(SHOWRSIZE * 100);
	...
	const int tempx = (lenw)-((_TexWnum / 2 * SCALED_TILE_SIZE) + static_cast<int>(sxue * float(SCALED_TILE_SIZE)));
	const int tempy = (lenh)-((_TexHnum / 2 * SCALED_TILE_SIZE) + static_cast<int>(syue * float(SCALED_TILE_SIZE)));
	...
	if(CMaskData::g_MaskData)
	{
		...
		fInf /= static_cast<float>((CMaskData::g_MaskData->iNumX) * (CMaskData::g_MaskData->iNumY));
		fInf *= 100;
		//id1 = (int)fInf * 100;
...
*/
		...
		g_cFindPathEx.SetDestDirection(static_cast<int>(CGameScene::GetMainCha()->GetPos().x), static_cast<int>(CGameScene::GetMainCha()->GetPos().y), mouse_coord.x, mouse_coord.y);
		g_cFindPathEx.SetTarget(static_cast<int>(CGameScene::GetMainCha()->GetPos().x), static_cast<int>(CGameScene::GetMainCha()->GetPos().y), mouse_coord.x, mouse_coord.y);
	...
	for(int  m = 0; m < _TexHnum; m++)
	{
		for(int n = 0; n < _TexWnum; n++)
		{
			...
			_vPicVer[0].m_vPos = D3DXVECTOR4(static_cast<float>(prc->left),static_cast<float>(prc->top),0.9f,1);
			_vPicVer[1].m_vPos = D3DXVECTOR4(static_cast<float>(prc->right),static_cast<float>(prc->top),0.9f,1);
			_vPicVer[2].m_vPos = D3DXVECTOR4(static_cast<float>(prc->right),static_cast<float>(prc->bottom),0.9f,1);
			_vPicVer[3].m_vPos = D3DXVECTOR4(static_cast<float>(prc->left),static_cast<float>(prc->bottom),0.9f,1);
	...
	if(CMPFont* pfont = g_pGameApp->GetFont(); pfont)
	{
