...
LRESULT CALLBACK LoadingProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
    {
        ...
        srand((unsigned)time(nullptr) * (g_nCurrentLogNo + 1));
...
void GameLoading::Create(string param)
{
    ...
    unsigned tid;
    _beginthreadex(nullptr, 0, LoadingThread, this, 0, &tid);
}

void GameLoading::Close()
{
    if (!szParam.empty()) return;
    RECT rc;
...
bool GameLoading::Active() const
{
    if (!szParam.empty()) return true;
