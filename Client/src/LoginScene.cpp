...
bool CLoginScene::_Init()
{
	...
	if (!IsLoad)
	{
		...
		if (pObj)
		{
			...
			//!g_stUISystem.m_sysProp.m_gameOption.bFramerate
...
void CLoginScene::_Connect()
{
	...
	//���û�еõ����ѡ��Gate��Ip
	int Gateport = 1973;
	if (!pszSelectGateIP) pszSelectGateIP = SelectGroupIP(m_iCurSelRegionIndex, m_iCurSelServerIndex, Gateport);
	...
	{
		CS_Connect(pszSelectGateIP, Gateport, g_Config.m_nConnectTimeOut);
	}
	//#endif
