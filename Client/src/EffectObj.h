...
public:
	CEffectBox() {
		_pDev = NULL; _lpIB = NULL; _lpVB = NULL;
		_lpIBLine = NULL;
	...
	void	Render()
	{
		...
		const auto val = _matScale * _matWorld;
		g_Render.SetTransformWorld(&val);
...
public:
	...
	CSceneNode* GetObjSceneNode() const { return _pObj; }
	int GetDummy() const { return _iDummy; }
	void SetIsWingEffect(const bool v) { isWingEffect = v; }
	bool GetisWingEffect() const { return isWingEffect; }
