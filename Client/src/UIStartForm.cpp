...
void CStartMgr::CleanDropListForm() const
{
...
void CStartMgr::SetMonsterInfo() const
{
	CleanDropListForm();

	const CChaRecord* charInfo = GetChaRecordInfo(MonstetDropID);
	if (!charInfo)
		return;
	...
	for (int i = 0; i < defCHA_INIT_ITEM_NUM; i++) {
		vect[i][0] = static_cast<int>(charInfo->lItem[i][0]);
		vect[i][1] = static_cast<int>(charInfo->lItem[i][1]);
		if (GetItemRecordInfo(vect[i][0]) == nullptr) {
			max = i;
			break;
		}
	}
	...
	for (int i = 0; i < max; i++) {
		...
		const auto rItem = new CItemCommand(rInfo);
		if (!rItem) 
			continue;	
		...
		_snprintf_s(get_name, sizeof get_name, _TRUNCATE, "%s", StringLimit(item_name, 16).c_str());
		LabMobItems[i]->SetCaption(get_name);

		float calcuDrop = (10000 / static_cast<float>(vect[i][1])) * g_DropBonus;
		if (calcuDrop > 100) {
			calcuDrop = 100;
		}

		char item_rate[25];
		_snprintf_s(item_rate, sizeof item_rate, _TRUNCATE, "%0.2f%%", calcuDrop);
	...
	const long chaLevel = g_pGameApp->GetCurScene()->GetMainCha()->getLv();
	const long mobLevel = charInfo->lLv;
	const long levelDif = chaLevel - mobLevel;
	double b = 1;
	...
	const double ExpAdd = floor(max(1, charInfo->lCExp / b)) * g_ExpBonus;
	char buf[64];
	
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lLv);
	LabMobLevel->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%0.f", ExpAdd);
	LabMobexp->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lMxHp);
	LabMobHP->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d/%d", charInfo->lMnAtk, charInfo->lMxAtk);
	LabMobAttack->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lHit);
	LabMobHitRate->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lFlee);
	LabMobDodge->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lDef);
	LabMobDef->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lPDef);
	LabMobPR->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lASpd);
	LabMobAtSpeed->SetCaption(buf);
	_snprintf_s(buf, sizeof buf, _TRUNCATE, "%d", charInfo->lMSpd);
...
void CStartMgr::SetTargetInfo(CCharacter* p_Target)
{
	if (!p_Target) {
		return;
	}
	...
	if (p_Target && !p_Target->IsPlayer()) {
		//render it once if same id 
		if (MonstetDropID == p_Target->getMobID())
			return;
		char chaLv[5];
		MonstetDropID = p_Target->getMobID();
		_snprintf_s(chaLv, sizeof chaLv, _TRUNCATE, "%d", p_Target->getLv());
		labTargetInfoName->SetCaption(p_Target->getName());
		targetInfoID = p_Target->getHumanID();
		RefreshTargetLifeNum(p_Target->getHP(), p_Target->getHPMax());
		labTargetLevel->SetCaption(chaLv);
		frmTargetInfo->Show();
		RefreshTargetModel(p_Target);
	}
	else {
		frmTargetInfo->Close();
	}
...
void CStartMgr::UpdateBackDrop() 
{
	...
	if (!pMain)
		return;
    const int nArea = CGameApp::GetCurScene()->GetTerrain()->GetTile(pMain->GetCurX() / 100, pMain->GetCurY() / 100)->getIsland();
    const auto world = dynamic_cast <CWorldScene*> (CGameApp::GetCurScene());
    ...
    if (nArea) {
		_snprintf_s(buf, sizeof buf, _TRUNCATE, "texture/ui/corsairs/npcBackdrop/2.tga", nArea);
    } else {
		_snprintf_s(buf, sizeof buf, _TRUNCATE, "texture/ui/corsairs/npcBackdrop/sea.tga");
    }
	CCompent* imgBackDropPlayer = g_stUIStart.frmDetail->Find("imgBackDropPlayer");
   CCompent*imgBackDropTarget = g_stUIStart.frmTargetInfo->Find("imgBackDropTarget");
   if (!imgBackDropPlayer || !imgBackDropTarget)
	   return;
   CCompent*teamBackDrops[4];
    ...
    for (int i = 0; i < 4; i++) {
		...
		_snprintf_s(formName, sizeof formName ,_TRUNCATE, "frmTeamMenber%d", i + 1);
		_snprintf_s(imgName, sizeof imgName, "imgBackDropTeam%d", i + 1);
        teamBackDrops[i] = _FindForm(formName)->Find(imgName);
    }
    ...
    if (GetFileAttributes(buf) == INVALID_FILE_ATTRIBUTES) {
        ...
        for (auto& teamBackDrop : teamBackDrops)
        {
			if (!teamBackDrop)
				continue;
	        teamBackDrop->GetImage()->LoadImage("texture/ui/corsairs/npcBackdrop/0.tga", 55, 44, 0, 0, 0);
        }
    } else {
        ...
        for (auto& teamBackDrop : teamBackDrops)
        {
			if(!teamBackDrop)
				continue;
	        teamBackDrop->GetImage()->LoadImage(buf, 55, 44, 0, 0, 0);
        }
    }
}
...
void CStartMgr::RefreshTargetLifeNum(long num, long max) 
{
    ...
    proTargetInfoHP->SetRange(0.0f, static_cast<float>(max));
    proTargetInfoHP->SetPosition(static_cast<float>(num));
    if (num == 0) {
        RemoveTarget();
    }
}
