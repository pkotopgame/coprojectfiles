...
protected:
    ...
    {   
        CServerGroupInfo *pInfo = static_cast<CServerGroupInfo*>(pRawDataInfo);
		...
		pInfo->port = Str2Int(ParamList[6]);
...
{
	if(nGroupNo>=GetCurServerGroupCnt(nRegionNo)) return nullptr;
...
// 通过组编号, 选中一个GateIP
inline const char *SelectGroupIP(const int nRegionNo, const int nGroupNo,int &gatePort)
{
