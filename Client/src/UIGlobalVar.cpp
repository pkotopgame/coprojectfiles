...
// maze portal timer @mothannakh
CPortalTime g_stUIPortalTime;
// boss respawn info@mothannakh
CBossRespawnInfo g_stBossRespawnInfo;
...
bool CUIInterface::All_Init()
{
	...
	for (auto& it : all)
	{
		if(const auto pUI = dynamic_cast<CUIInterface*>(it); !pUI->Init() )
		{
...
void CUIInterface::_evtDragToGoodsEvent(CGuiData *pSender, CCommandObj* pItem, int nGridID, bool& isAccept)
{
    ...
    if( COneCommand* pCom = dynamic_cast<COneCommand*>(CDrag::GetParent()); pCom)
	{
        int iIndex = -1;
		...
		if (pSelf == g_stUIEquip.GetGoodsGrid()	&& g_stUIMakeEquip.IsAllCommand(pCom))
		{
			...
			{
				const int iIndex = g_stUIMakeEquip.GetItemComIndex(pCom);
				if (iIndex != -1)
		...
		// �ᴿ
		// ���ᴿ�����ϵ���Ʒ��
        const int nPurifyIndex = g_stUIPurify.GetItemComIndex(pCom);
...
bool CUIInterface::_evtESCKey(char& key)
{
	if( key==VK_ESCAPE )
	{
		...
		if( frm ) 
		{
			const GuiFormEscCloseEvent	evtEscClose = frm->evtEscClose;
			if( evtEscClose)
			{
				evtEscClose( frm );
			}
