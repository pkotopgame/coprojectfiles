...
void CHeadSay::RenderStateIcons(CCharacter* cha, int x, int y, float scale, float spacing, int rowSize, bool Rendertimer) {
		...
		if (cha->GetStateMgr()->HasSkillState(i)) {
			...
			if (pState) {
				...
				if (RenderIcon) {
					...
					const int xi = x + ((stateCount % rowSize) * spacing);
					const int yi = y + ((stateCount / rowSize) * (spacing + yspace));
					stateIcon.Render(xi, yi);


					const CChaStateMgr::stChaState stateData = cha->GetStateMgr()->GetStateData(i);
					...
					const int xm = g_pGameApp->GetMouseX();
					const int ym = g_pGameApp->GetMouseY();
	...
	if (stateCount > 0 && g_IsShowStates) {
		int w = 32 * stateCount;
		const int h = stateCount <= 9 ? 45 : stateCount >= 18 ? 120 : 94;
		if (w > 250) w = 250;
...
void CHeadSay::Render(const D3DXVECTOR3& pos)
{
	static int x = 0, y = 0;
	...
	*/
	...
	if (_ShowBars && (_IsShowLife || (_pOwn->IsPlayer() && _ShowInfo)) && !_fLifeW <= 0) //Ѫ��
	{
		...
		const int nLifeWidth = _pImgLife->GetWidth();
		_pImgLife->SetScaleW(1, _fLifeW);
		const int nOffset = static_cast<int>((x - g_Render.GetScrWidth() / 2) * 0.02f);
		...
		if (_pOwn->GetStateMgr()->HasSkillState(83)) {
			if (isTeamMember || isGuildMember || (_ShowInfo && _pOwn->IsPlayer()) || _IsShowLife) {
				...
				if (_pOwn->IsPlayer()) {
					const int nManaWidth = _pImgMana->GetWidth();
					_pImgMana->SetScaleW(1, _fManaW);

					DWORD spcolour = 0xA0FF0000;
					if (_fManaW > 0) {
						const int red = 255 - (_fManaW * 255);
						const int blue = _fManaW * 255;
		...
		if (g_IsShowStates) {
			constexpr float scale = 0.75;
			constexpr float spacing = scale * 32 + 2;
    ...
    {
#ifdef _LOG_NAME_                     //��ɫ����
	    constexpr int LINE_HEIGHT_STEP = 14;		//һ�еĸ߶�
...
//_dwNameColor
        ...
        else if( _IsShowName || g_IsShowNames || (_pOwn->GetIsPK() && _ShowEnemyNames ) || (_ShowInfo && _pOwn->IsPlayer()) )
        {
			...
			if ( *(_pOwn->getGuildName()) )
			{
				...
				constexpr int iGuildNameHeightStep = LINE_HEIGHT_STEP;
				...
				for (const auto& i : s_szConsortiaNamePart)
				{
					strncat( s_szConsortiaName, i, NAME_LENGTH );
				}
				...
				// renderÿһ������
				if(const int perm = (_pOwn->getGuildPermission()&emGldPermLeader); perm == emGldPermLeader){
				...
				for (const auto& i : s_szConsortiaNamePart)
				{
					CGuiFont::s_Font.BRender(i, x + iStartPosX, y - iGuildNameHeightStep, _pOwn->getGuildColor(), COLOR_BLACK );
					iStartPosX += CGuiFont::s_Font.GetWidth(i);
				}
			...
			for (const auto& i : s_sNamePart)
			{
				strncat( s_szName, i, NAME_LENGTH );
			}
    ...
    else if( _nShowTime > 0 )                        //����
    {
		...
		if ( _pObj )	
        {
	        const int  left     =  x- (int)( _fScale * _pObj->GetWidth()/2) ;
	        const int  nLine    = _pObj->GetLineNum();

	        const string str = _pObj->GetString();
    ...
    if( CCharacter::IsShowLogName )
    {
		...
		const int nCount = pState->GetSkillStateNum();
