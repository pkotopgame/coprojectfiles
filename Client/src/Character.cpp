...
{
	for (auto& cha : std::span{ g_pGameApp->GetCurScene()->_pChaArray, static_cast<unsigned>(g_pGameApp->GetCurScene()->_nChaCnt) })
	{
		if (cha.GetIsForUI()) {
			continue;
		}
...
bool CCharacter::UpdataItem(int nItem, DWORD nLink)
{
	...
	case enumEQUIP_GLOVE:
	case enumEQUIP_SHOES: {
		if (_pDefaultChaInfo == nullptr) {
			LG(getLogName(), "msgChangePart Error, DefaultChaInfo is null\n\n");
			return false;
		}
		if (nItem == 0) {
			nItem = _pDefaultChaInfo->sSkinInfo[nLink];
			...
			bool checkHair = false;
			if (nLink == enumEQUIP_HEAD) {
				if (CItemRecord* pInfoMain = GetItemRecordInfo(nItem)) {
					if (pInfoMain->sType != 28) {
						checkHair = true;
					}
				}
			}
			const stNetChangeChaPart& part = GetPart();
			int appID = GetApparelID(part.SLink[nLink + 19]);
