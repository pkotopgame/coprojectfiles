...
public:
	...
	bool			Create(IDirect3DDeviceX* pDev, const RECT& rcwnd, CGameScene* pScene = nullptr,int bksize = 256);

	//void			Render();

	void			MoveTo(const D3DXVECTOR3 &vPos,D3DXVECTOR3&	vCameraDir,float fCameraAngle,float fCameradDist,int iAngle = 0);
	
	void			SetDist(const float fDist)	{_fDist = fDist;_bUpdate = true;_bMove = true;}
	float			GetDist() const { return _fDist;}

	void			SetUpdate(const bool bUpdate){ _bUpdate = bUpdate;}
	virtual	void	FrameMove(DWORD dwDailTime);
	virtual void	Render();

	void			GetWndPos(int &nOutX, int &nOutY, const D3DXVECTOR3 &vWorldPos) const
	{
		
		D3DXVECTOR4 vOut;
		const auto v = _matView * _matProj;
		D3DXVec3Transform(&vOut, &vWorldPos, &(v));
		nOutX = static_cast<int>(static_cast<float>(_rcWnd.right - _rcWnd.left) * (vOut.x / vOut.w * 0.5f + 0.5f));
		nOutY = static_cast<int>(static_cast<float>(_rcWnd.bottom - _rcWnd.top) * (0.5f - vOut.y / vOut.w * 0.5f));
...
struct M2D_AVER
{
	...
	DWORD m_color;
	D3DXVECTOR2 m_fUV;
	void SetValue(const D3DXVECTOR4 &vPos, const DWORD color, const D3DXVECTOR2 &fUV)
	{
		m_vPos = vPos;
		m_color = color;
		m_fUV = fUV;
...
public:
	...
	BOOL    Init(IDirect3DDeviceX* pDev)
	{
		...
		_vers[0] = D3DXVECTOR4(-frad, -frad,  0.000001f,  static_cast<float>(1));
		_vers[1] = D3DXVECTOR4(frad, -frad,   0.000001f,  static_cast<float>(1));
		_vers[2] = D3DXVECTOR4(frad, frad,    0.000001f, static_cast<float>(1));
		_vers[3] = D3DXVECTOR4(-frad, frad,   0.000001f,  static_cast<float>(1));
		...
		{
#ifdef MGR
			if(FAILED(lwLoadTex(&_pTex[n],res_mgr, pszName[n], nullptr, D3DFMT_A4R4G4B4)))
			{
	...
	void	setAngle(const int z)
	{
		D3DXMatrixRotationZ(&_matRotat,Angle2Radian(static_cast<float>(z)) + D3DX_PI);
	}
	void	setScaling(const float x, const float y, const float z)
	{
	...
	void	setTexture(const int iID)
	{
		_iCurTex = iID;
	}
	void	Begin(IDirect3DVertexShaderX* dwVs) const
	{
#ifdef MGR
		_pVB->BeginSet();
	...
	void	RenderWndCha(const int iType, const int nx, const int ny, const int wh = 8)
	{
		RECT rc;
		//int nx,ny;
		...
		rc.left = static_cast<LONG>(nx);
		rc.top = static_cast<LONG>(ny);
		rc.right = rc.left + wh;
		rc.bottom = rc.top + wh;
...
public:
	...
	struct TEXIDX
	{
		...
		TEXIDX(const int tx, const int ty)
		{
			x = tx;
			y = ty;
			pTex = nullptr;
...
public:
	...
	void	Create(const RECT& rc);

	void    UpdataFace(const stNetTeamChaPart& stPart, bool IsPlayer = true);
...
public:
	...
	void	setPos(const int posx, const int posy, const int mainx, const int mainy, const int wndx, const int wndy, const int wndw = 64)
	{
		int nx, ny;
		int tx,ty;
		//nx =  int((posx - mainx) / 100.0f  * 1.6f);
		//ny =  int((posy - mainy) / 100.0f  * 1.6f);
		nx =  static_cast<int>(static_cast<float>(posx / 100) * 1.6f);
		ny =  static_cast<int>(static_cast<float>(posy / 100) * 1.6f);
		tx =  static_cast<int>(static_cast<float>(mainx / 100) * 1.6f);
		ty =  static_cast<int>(static_cast<float>(mainy / 100) * 1.6f);

		rc.left = static_cast<LONG>((nx - tx) + wndw);
		rc.top = static_cast<LONG>((ny - ty) + wndw);
		rc.right = rc.left + nw;
		rc.bottom = rc.top + nh;
	...
	void    setPos(const int posx, const int posy, const int wndx, const int wndy)
	{
		mposx = posx + wndx;
		mposy = posy + wndy;

		rc.left = static_cast<LONG>(posx) - nw / 2;
		rc.top = static_cast<LONG>(posy) - nh / 2;
		rc.right = rc.left + nw;
		rc.bottom = rc.top + nh;
	}
	void	setScaling(const int x, const int y)
	{
		nw = x;
		nh = y;

		rc.left = static_cast<LONG>(mposx) - nw / 2;
		rc.top = static_cast<LONG>(mposy) - nh / 2;
		rc.right = rc.left + nw;
		rc.bottom = rc.top + nh;
	}
	void	setAngle(const int z)
	{
		...
		{
			D3DXVECTOR2 vpos[4];
			vpos[0] = D3DXVECTOR2(static_cast<float>(rc.left),static_cast<float>(rc.top));
			vpos[1] = D3DXVECTOR2(static_cast<float>(rc.right),static_cast<float>(rc.top));
			vpos[2] = D3DXVECTOR2(static_cast<float>(rc.right),static_cast<float>(rc.bottom));
			vpos[3] = D3DXVECTOR2(static_cast<float>(rc.left),static_cast<float>(rc.bottom));

			int n;
			D3DXMATRIX mat;
			//D3DXMatrixRotationZ(&mat,Angle2Radian(float(z)));
			D3DXVECTOR3	tvpos(static_cast<float>(mposx),static_cast<float>(mposy),0);
			 auto v = D3DXVECTOR3(0, 0, 1);
			GetMatrixRotation(&mat, &tvpos, &v, Angle2Radian(static_cast<float>(z)) + D3DX_PI);
...
public:
	...
	bool	GetMask(const int x, const int y)
	{
...
{
public:
	CMinimap(): _vWndVer{}, _vPicVer{}, _sx(0), _sy(0)
	{
		_pTexMask = nullptr;
	...
	struct TEXIDX
	{
		...
		BOOL operator == ( CONST TEXIDX& tes ) const
		{
			return ((x == tes.x) && (y == tes.y));
		}
		int	x,y;
...
{
public:
	CLargerMap(): _vPicVer{}, _sx(0), _sy(0), fInf(0), _nCenterX(0), _nCenterY(0), _nInitX(0), _nInitY(0)
	{
	}
	...
	~CLargerMap() override
	{
		//if(_pTex)
		{
			for (int m = 0; m < _TexHnum * _TexWnum; m++)
			{
	...
	void SetFollow(const bool v) { follow_player = v; }
...
protected:
	...
	void	GetChangeNameText(const char* pszName)
	{
