...
void CMagicEff::_UpdateValid(BOOL bValid)
{
	if (!bValid)
	{
		...
		//SetFontEffect("20", NULL);//TODO: DX9
		_bPlay = false;
	}
	else
	{
...
{
	float fX = static_cast<float>(_nCurX) / 100.0f;
	float fY = static_cast<float>(_nCurY) / 100.0f;
	auto val = D3DXVECTOR3(fX, fY, _fHei + static_cast<float>(_nHeightOff) / 100.0f);
	MoveTo(&val);
}
...
void		CNavigationBar::Render()
{
	...
	vPos.x = static_cast<float>(static_cast<int>(vPos.x));
	vPos.y = static_cast<float>(static_cast<int>(vPos.y));
	const auto val = _vTarget - vPos;
	D3DXVec3Normalize(&vDir, &val);
	vDir.z = 0;
	...
	else
	{
		...
		const float fDist = D3DXVec3Length(&v[0]);
		fangle = acosf(D3DXVec3Dot(&v[0],
			&v[1]) /
			fDist);
		if (vDir.x >= 0.0f)
			fangle = -fangle;
