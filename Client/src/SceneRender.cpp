...
void CGameScene::_Render()
{
	...
	if (g_Config.m_bEditor)
	{
		if (_vMousePos.x > 0)
		{
			...
			{
				auto x = D3DXVECTOR3(_vMousePos.x, _vMousePos.y, _vMousePos.z + 4.0f);
				g_Render.AddLine(_vMousePos,x, 0xFFFF0000);
			}
		}

		// µ±Ç°½ÇÉ«Èç¹ûÊÇÑ²Âß¹Ö, Ôò°ÑÑ²ÂßÂ·Ïß»æÖÆ³öÀ´
		...
		for (int i = 0; i < GetChaCnt(); i++)
		{
			...
			if (pCha->IsValid())
			{
				if (pCha->getPatrolX() > 0)
				{
					...
					auto zz = vec2 + D3DXVECTOR3(0.1f, 0.2f, 0.0f);
					g_Render.AddLine(vec1, zz, 0xFFFF0000);
				}
			}
		}
	}
...
#define OPEN_AMBIENT_ENV
	...
	//lemon add@2004.12.14 ·ÀÖ¹Á÷¹âÈËÎïÔÚÌØÐ§Ó°ÏìÏÂ¸Ä±äÍ¸Ã÷¶È
