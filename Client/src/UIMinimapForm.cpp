...
void CMiniMapMgr::_MiniFormMouseEvent(CCompent *pSender, int nMsgType, int x, int y, DWORD dwKey)
{
	...
	{
		CProCirculateCS* proCir = static_cast<CProCirculateCS*>(g_NetIF->GetProCir());
		proCir->OpenRankings();
	}
	...
	else if (name == "btnBossRespawnTimer") // button for portal timer   //
	{
		if (CForm* f = CFormMgr::s_Mgr.Find("frmBossRespawn")) {
			if (!f->GetIsShow()) {
				if (const auto pCha = CGameScene::GetMainCha(); pCha) {
					if (static DWORD time = 0; CGameApp::GetCurTick() > time) {
						CS_BeginAction(pCha, enumACTION_BossRespawnInfo, nullptr);
						time = CGameApp::GetCurTick() + 30000; //half min cooldown 
					}
