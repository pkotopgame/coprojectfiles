...
void CProCirculateCS::BeginAction( CCharacter* pCha, DWORD type, void* param, CActionState* pState )
{
	...
	try
	{
		...
		case	enumACTION_KITBAGTMP_DRAG:	// �϶���ʱ����
			...
			// boss respawn timer btn
		case enumACTION_BossRespawnInfo: {
			pCNetIf->SendPacketMessage(pk);
			break;
		}
...
bool CProCirculate::SendPrivateKey() const
{
	const CryptoPP::RSAES_OAEP_SHA_Encryptor e(g_NetIF->srvPublicKey);
	...
	//g_NetIF->_comm_enc = g_Config.EncryptPacket;
...
void CProCirculate::Login(const char *accounts,const char *password, const char* passport) const
{
