...
RenderStateMgr::RenderStateMgr()
	: _dev_obj(nullptr), _rsa_scene(nullptr), _rsa_cha(nullptr), _rsa_sceneobj(nullptr),
	_rsa_sceneitem(nullptr), _rsa_terrain(nullptr), _rsa_transpobj(nullptr) {
}
...
HRESULT RenderStateMgr::Init(MPIDeviceObject* dev_obj)
{
...

	{
		...
		const D3DCAPSX* dev_caps = dev_obj->GetDeviceCaps();
		...
		D3DXVec3Normalize(static_cast<D3DXVECTOR3*>(&_scnobj_lgt.Direction), static_cast<D3DXVECTOR3*>(&_scnobj_lgt.Direction));
	...
	// begin _rsa_cha
	{
		...
		D3DXVec3Normalize(static_cast<D3DXVECTOR3*>(&_cha_lgt.Direction), static_cast<D3DXVECTOR3*>(&_cha_lgt.Direction));
