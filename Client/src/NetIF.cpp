...
BOOL NetIF::HandlePacketMessage(DataSocket *datasock,LPRPACKET pk)
{
	...
	case CMD_PC_REFRESH_SELECT: return SC_RefreshSelectScreen(pk);
		//@mothannakh
	case CMD_MC_BossTimer: {
		g_stBossRespawnInfo.ClearList(); // we clear old list before add new one
		for (auto totalbosses = pk.ReadShort(); totalbosses > 0; --totalbosses) {
			const auto BossName = pk.ReadString();
			const auto MapName = pk.ReadString();
			const auto Location = pk.ReadString();
			const auto NextRespawn = pk.ReadLong();
			const auto MonsterStates = pk.ReadString();
			std::string MapAreaName = MapName;
			if (const auto& Map = GetMapInfo(MapName); Map) {
				MapAreaName = Map->szName;
			}
			std::string connectLocation = MapAreaName + Location;
			g_stBossRespawnInfo.Add({ BossName, connectLocation, NextRespawn, MonsterStates });
			// LG("monsterInfo", "%s %s %ld %s\n", BossName, Location, NextRespawn, MonsterStates);
		}
		return TRUE;
	}
