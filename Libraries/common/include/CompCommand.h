...
using LOOK = struct stNetChangeChaPart // Change the appearance of the character.Note: The client-server default skin convention: When the client receives a skin of 0, the skin in the CharacterRecord is automatically loaded.
{
	stNetChangeChaPart() {
		sVer = 0;
		sTypeID = 0;
		...
		sCannon = 0;
		sEquipment = 0;
		//force links zero after upgrade
	...
	union {
		struct {
			...
			short sHairID; // Default hair and face
		};

		struct {
			USHORT sPosID;     // Ship action ID
			USHORT sBoatID;    // Ship Information Form ID
			USHORT sHeader;    // Bow
			USHORT sBody;      // Hull
			USHORT sEngine;    // Boat motor
			USHORT sCannon;    // Ship artillery
			USHORT sEquipment; // Ship equipment
		};
	};
};
...
struct Look_Minimal
{
	...
	explicit Look_Minimal(LOOK const& look)
	{
		hairID = look.sHairID;
		typeID = look.sTypeID;

		std::ranges::transform(look.SLink, std::begin(equip_IDs),
